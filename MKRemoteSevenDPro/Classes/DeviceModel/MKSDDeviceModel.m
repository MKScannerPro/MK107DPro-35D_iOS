...
NSString *const MKSDDeviceModelOfflineNotification = @"MKSDDeviceModelOfflineNotification";

@interface MKSDDeviceModel ()
...
@implementation MKSDDeviceModel

- (void)dealloc{
    NSLog(@"MKSDDeviceModel销毁");
}

#pragma mark - public method

- (void)startStateMonitoringTimer{
    if (self.onLineState == MKSDDeviceModelStateOffline) {
        return;
    }
    ...
    dispatch_source_set_event_handler(self.receiveTimer, ^{
        ...
        if (self.receiveTimerCount >= 62.f) {
            ...
            self.receiveTimerCount = 0;
            self.onLineState = MKSDDeviceModelStateOffline;
            dispatch_async(dispatch_get_main_queue(), ^{
                [[NSNotificationCenter defaultCenter] postNotificationName:MKSDDeviceModelOfflineNotification
                                                                    object:nil
                                                                  userInfo:@{@"macAddress":self.macAddress}];
                if ([self.delegate respondsToSelector:@selector(sd_deviceOfflineWithMacAddress:)]) {
                    [self.delegate sd_deviceOfflineWithMacAddress:self.macAddress];
                }
            });
            return ;
        }
...
- (NSString *)currentSubscribedTopic {
    if (ValidStr([MKSDMQTTDataManager shared].serverParams.publishTopic)) {
        return [MKSDMQTTDataManager shared].serverParams.publishTopic;
    }
...
- (NSString *)currentPublishedTopic {
    if (ValidStr([MKSDMQTTDataManager shared].serverParams.subscribeTopic)) {
        return [MKSDMQTTDataManager shared].serverParams.subscribeTopic;
    }
