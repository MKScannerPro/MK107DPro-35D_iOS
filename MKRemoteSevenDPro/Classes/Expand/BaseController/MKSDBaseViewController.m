...
@interface MKSDBaseViewController ()

@end

@implementation MKSDBaseViewController

- (void)dealloc {
    NSLog(@"MKSDBaseViewController销毁");
...
- (void)loadChanged:(NSNotification *)note {
    ...
    if (![user[@"device_info"][@"mac"] isEqualToString:[MKSDDeviceModeManager shared].macAddress] || ![MKBaseViewController isCurrentViewControllerVisible:self]) {
        return;
    }
...
- (void)addNotifications {
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(deviceOffline:)
                                                 name:MKSDDeviceModelOfflineNotification
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(loadChanged:)
                                                 name:MKSDReceiveLoadChangeNotification
                                               object:nil];
}

- (void)processOfflineWithMacAddress:(NSString *)macAddress {
    if (![macAddress isEqualToString:[MKSDDeviceModeManager shared].macAddress]) {
        return;
    }
