...
@implementation MKSDFilterBeaconCellModel
@end

@interface MKSDFilterBeaconCell ()
...
@implementation MKSDFilterBeaconCell

+ (MKSDFilterBeaconCell *)initCellWithTableView:(UITableView *)tableView {
    MKSDFilterBeaconCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKSDFilterBeaconCellIdenty"];
    if (!cell) {
        cell = [[MKSDFilterBeaconCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKSDFilterBeaconCellIdenty"];
    }
    return cell;
}
...
#pragma mark - setter
- (void)setDataModel:(MKSDFilterBeaconCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKSDFilterBeaconCellModel.class]) {
        return;
    }
...
- (MKTextField *)minTextField {
    if (!_minTextField) {
        ...
        _minTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(mk_sd_beaconMinValueChanged:index:)]) {
                [self.delegate mk_sd_beaconMinValueChanged:text index:self.dataModel.index];
            }
        };
    }
...
- (MKTextField *)maxTextField {
    if (!_maxTextField) {
        ...
        _maxTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(mk_sd_beaconMaxValueChanged:index:)]) {
                [self.delegate mk_sd_beaconMaxValueChanged:text index:self.dataModel.index];
            }
        };
    }
