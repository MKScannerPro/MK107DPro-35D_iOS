...
@implementation MKSDUserCredentialsViewModel
@end

@interface MKSDUserCredentialsView ()
...
@implementation MKSDUserCredentialsView
...
#pragma mark - setter
- (void)setDataModel:(MKSDUserCredentialsViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKSDUserCredentialsViewModel.class]) {
        return;
    }
...
- (MKTextField *)userNameTextField {
    if (!_userNameTextField) {
        ...
        _userNameTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(sd_mqtt_userCredentials_userNameChanged:)]) {
                [self.delegate sd_mqtt_userCredentials_userNameChanged:text];
            }
        };
...
- (MKTextField *)passwordTextField {
    if (!_passwordTextField) {
        ...
        _passwordTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(sd_mqtt_userCredentials_passwordChanged:)]) {
                [self.delegate sd_mqtt_userCredentials_passwordChanged:text];
            }
        };
