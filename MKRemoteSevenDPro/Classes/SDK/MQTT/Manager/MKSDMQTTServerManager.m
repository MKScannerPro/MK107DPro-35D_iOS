...
static MKSDMQTTServerManager *manager = nil;
static dispatch_once_t onceToken;

@interface NSObject (MKSDMQTTServerManager)

@end

@implementation NSObject (MKSDMQTTServerManager)

//+ (void)load{
//    [MKSDMQTTServerManager shared];
//}

@end

@interface MKSDMQTTServerManager ()

@property (nonatomic, assign)MKSDMQTTSessionManagerState state;

@property (nonatomic, strong)MKSDServerParamsModel *paramsModel;

@property (nonatomic, strong)NSMutableArray <id <MKSDServerManagerProtocol>>*managerList;

@property (nonatomic, strong)NSMutableDictionary *subscriptions;

@end

@implementation MKSDMQTTServerManager
...
+ (MKSDMQTTServerManager *)shared {
    dispatch_once(&onceToken, ^{
        if (!manager) {
            manager = [MKSDMQTTServerManager new];
        }
    });
    return manager;
}
...
- (void)sessionManager:(MQTTSessionManager *)sessionManager
    ...
    @synchronized (self.managerList) {
        for (id <MKSDServerManagerProtocol>protocol in self.managerList) {
            if ([protocol respondsToSelector:@selector(sd_didReceiveMessage:onTopic:)]) {
                [protocol sd_didReceiveMessage:dataDic onTopic:topic];
            }
        }
    }
}
...
- (void)sessionManager:(MQTTSessionManager *)sessionManager didChangeState:(MKMQTTSessionManagerState)newState {
    ...
    @synchronized (self.managerList) {
        for (id <MKSDServerManagerProtocol>protocol in self.managerList) {
            if ([protocol respondsToSelector:@selector(sd_didChangeState:)]) {
                [protocol sd_didChangeState:self.state];
            }
        }
    }
}
...
- (void)networkStateChanged{
    ...
    if (![[MKNetworkManager sharedInstance] currentNetworkAvailable]) {
        ...
        [self sessionManager:manager didChangeState:MKSDMQTTSessionManagerStateStarting];
        return;
    }
...
- (void)loadDataManager:(nonnull id <MKSDServerManagerProtocol>)dataManager {
    @synchronized (self.managerList) {
        if (dataManager
            && [dataManager conformsToProtocol:@protocol(MKSDServerManagerProtocol)]
...
- (BOOL)removeDataManager:(nonnull id <MKSDServerManagerProtocol>)dataManager {
    @synchronized (self.managerList) {
        if (!dataManager ||
            ![dataManager conformsToProtocol:@protocol(MKSDServerManagerProtocol)] ||
            ![self.managerList containsObject:dataManager]) {
...
- (void)loadLogFile {
    ...
    NSString *recordDateString = [[NSUserDefaults standardUserDefaults] objectForKey:@"mk_sd_mqtt_recordDate"];
    if (!ValidStr(recordDateString)) {
        //如果第一次存，则把当天的日期写入
        [[NSUserDefaults standardUserDefaults] setObject:[dateFormatter stringFromDate:[NSDate date]] forKey:@"mk_sd_mqtt_recordDate"];
        return;
    }
    //写入当天日期，用来做比较
    [[NSUserDefaults standardUserDefaults] setObject:[dateFormatter stringFromDate:[NSDate date]] forKey:@"mk_sd_mqtt_recordDate"];
...
#pragma mark - getter
- (MKSDServerParamsModel *)paramsModel {
    if (!_paramsModel) {
        _paramsModel = [[MKSDServerParamsModel alloc] init];
    }
    return _paramsModel;
}

- (NSMutableArray<id<MKSDServerManagerProtocol>> *)managerList {
    if (!_managerList) {
