...
static NSString *const mk_sd_logName = @"mk_sd_bleLog";

NSString *const mk_sd_peripheralConnectStateChangedNotification = @"mk_sd_peripheralConnectStateChangedNotification";
NSString *const mk_sd_centralManagerStateChangedNotification = @"mk_sd_centralManagerStateChangedNotification";

NSString *const mk_sd_deviceDisconnectTypeNotification = @"mk_sd_deviceDisconnectTypeNotification";

static MKSDCentralManager *manager = nil;
static dispatch_once_t onceToken;

@interface MKSDCentralManager ()
...
@property (nonatomic, assign)mk_sd_centralConnectStatus connectStatus;

@end

@implementation MKSDCentralManager

- (void)dealloc {
    [self logToLocal:@"MKSDCentralManager销毁"];
    NSLog(@"MKSDCentralManager销毁");
}

- (instancetype)init {
    if (self = [super init]) {
        [self logToLocal:@"MKSDCentralManager初始化"];
...
+ (MKSDCentralManager *)shared {
    dispatch_once(&onceToken, ^{
        if (!manager) {
            manager = [MKSDCentralManager new];
        }
    });
    return manager;
}
...
- (void)MKBLEBaseCentralManagerDiscoverPeripheral:(CBPeripheral *)peripheral
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if ([self.delegate respondsToSelector:@selector(mk_sd_receiveDevice:)]) {
                [self.delegate mk_sd_receiveDevice:dataModel];
            }
        });
    });
}
...
- (void)MKBLEBaseCentralManagerStartScan {
    ...
    if ([self.delegate respondsToSelector:@selector(mk_sd_startScan)]) {
        [self.delegate mk_sd_startScan];
    }
}

- (void)MKBLEBaseCentralManagerStopScan {
    [self logToLocal:@"停止扫描"];
    if ([self.delegate respondsToSelector:@selector(mk_sd_stopScan)]) {
        [self.delegate mk_sd_stopScan];
    }
}
...
- (void)MKBLEBaseCentralManagerStateChanged:(MKCentralManagerState)centralManagerState {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:mk_sd_centralManagerStateChangedNotification object:nil];
}
...
- (void)MKBLEBasePeripheralConnectStateChanged:(MKPeripheralConnectState)connectState {
    ...
    if (connectState == MKPeripheralConnectStateUnknow) {
        self.connectStatus = mk_sd_centralConnectStatusUnknow;
    }else if (connectState == MKPeripheralConnectStateConnecting) {
        self.connectStatus = mk_sd_centralConnectStatusConnecting;
    }else if (connectState == MKPeripheralConnectStateConnectedFailed) {
        self.connectStatus = mk_sd_centralConnectStatusConnectedFailed;
    }else if (connectState == MKPeripheralConnectStateDisconnect) {
        self.connectStatus = mk_sd_centralConnectStatusDisconnect;
    }
    NSLog(@"当前连接状态发生改变了:%@",@(connectState));
    NSString *string = [NSString stringWithFormat:@"连接状态发生改变:%@",@(connectState)];
    [self logToLocal:string];
    [[NSNotificationCenter defaultCenter] postNotificationName:mk_sd_peripheralConnectStateChangedNotification object:nil];
}
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error {
    ...
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@"AA01"]]) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:mk_sd_deviceDisconnectTypeNotification
                                                            object:nil
...
- (mk_sd_centralManagerStatus )centralStatus {
    ...
    ? mk_sd_centralManagerStatusEnable
    : mk_sd_centralManagerStatusUnable;
}
...
- (void)addTaskWithTaskID:(mk_sd_taskOperationID)operationID
    ...
    MKSDOperation <MKBLEBaseOperationProtocol>*operation = [self generateOperationWithOperationID:operationID
...
- (void)addReadTaskWithTaskID:(mk_sd_taskOperationID)operationID
               characteristic:(CBCharacteristic *)characteristic
                 successBlock:(void (^)(id returnData))successBlock
                 failureBlock:(void (^)(NSError *error))failureBlock {
    MKSDOperation <MKBLEBaseOperationProtocol>*operation = [self generateReadOperationWithOperationID:operationID
                                                                                       characteristic:characteristic
                                                                                         successBlock:successBlock
                                                                                         failureBlock:failureBlock];
    if (!operation) {
...
- (void)connectPeripheral:(CBPeripheral *)peripheral
    ...
    MKSDPeripheral *trackerPeripheral = [[MKSDPeripheral alloc] initWithPeripheral:peripheral];
...
- (void)sendPasswordToDevice {
    ...
    __weak typeof(self) weakSelf = self;
    MKSDOperation *operation = [[MKSDOperation alloc] initOperationWithID:mk_sd_connectPasswordOperation commandBlock:^{
        [[MKBLEBaseCentralManager shared] sendDataToPeripheral:commandData characteristic:[MKBLEBaseCentralManager shared].peripheral.sd_password type:CBCharacteristicWriteWithResponse];
        ...
        //密码正确
        MKBLEBase_main_safe(^{
            sself.connectStatus = mk_sd_centralConnectStatusConnected;
            [[NSNotificationCenter defaultCenter] postNotificationName:mk_sd_peripheralConnectStateChangedNotification object:nil];
            if (sself.sucBlock) {
...
- (MKSDOperation <MKBLEBaseOperationProtocol>*)generateOperationWithOperationID:(mk_sd_taskOperationID)operationID
                                                                 characteristic:(CBCharacteristic *)characteristic
                                                                    commandData:(NSString *)commandData
                                                                   successBlock:(void (^)(id returnData))successBlock
                                                                   failureBlock:(void (^)(NSError *error))failureBlock{
    ...
    __weak typeof(self) weakSelf = self;
    MKSDOperation <MKBLEBaseOperationProtocol>*operation = [[MKSDOperation alloc] initOperationWithID:operationID commandBlock:^{
        __strong typeof(self) sself = weakSelf;
...
- (MKSDOperation <MKBLEBaseOperationProtocol>*)generateReadOperationWithOperationID:(mk_sd_taskOperationID)operationID
                                                                     characteristic:(CBCharacteristic *)characteristic
                                                                       successBlock:(void (^)(id returnData))successBlock
                                                                       failureBlock:(void (^)(NSError *error))failureBlock{
    if (![[MKBLEBaseCentralManager shared] readyToCommunication]) {
...
- (NSDictionary *)parseModelWithRssi:(NSNumber *)rssi advDic:(NSDictionary *)advDic peripheral:(CBPeripheral *)peripheral {
    ...
    if (![deviceType isEqualToString:@"40"]) {
        return @{};
    }
...
- (void)logToLocal:(NSString *)string {
    ...
    [MKBLEBaseLogManager saveDataWithFileName:mk_sd_logName dataList:@[string]];
}
