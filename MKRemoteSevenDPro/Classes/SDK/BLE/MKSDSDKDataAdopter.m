...
@implementation MKSDSDKDataAdopter

+ (NSString *)fetchWifiSecurity:(mk_sd_wifiSecurity)security {
    switch (security) {
        case mk_sd_wifiSecurity_personal:
            return @"00";
        case mk_sd_wifiSecurity_enterprise:
            return @"01";
    }
}
...
+ (NSString *)fetchWifiEapType:(mk_sd_eapType)eapType {
    switch (eapType) {
        case mk_sd_eapType_peap_mschapv2:
            return @"00";
        case mk_sd_eapType_ttls_mschapv2:
            return @"01";
        case mk_sd_eapType_tls:
            return @"02";
    }
}
...
+ (NSString *)fetchConnectModeString:(mk_sd_connectMode)mode {
    switch (mode) {
        case mk_sd_connectMode_TCP:
            return @"00";
        case mk_sd_connectMode_CASignedServerCertificate:
            return @"01";
        case mk_sd_connectMode_CACertificate:
            return @"02";
        case mk_sd_connectMode_SelfSignedCertificates:
            return @"03";
    }
}
...
+ (NSString *)fetchMqttServerQosMode:(mk_sd_mqttServerQosMode)mode {
    switch (mode) {
        case mk_sd_mqttQosLevelAtMostOnce:
            return @"00";
        case mk_sd_mqttQosLevelAtLeastOnce:
            return @"01";
        case mk_sd_mqttQosLevelExactlyOnce:
            return @"02";
    }
}
