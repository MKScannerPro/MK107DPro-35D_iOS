...
@implementation MKSDCreatScannerProDeviceModel
...
@interface MKSDNetworkService ()

@property (nonatomic, strong)NSURLSessionDataTask *addScannerProDeviceTask;

@end

@implementation MKSDNetworkService

+ (instancetype)share {
    static dispatch_once_t t;
    static MKSDNetworkService *service = nil;
    dispatch_once(&t, ^{
        service = [[MKSDNetworkService alloc] init];
    });
    return service;
}

- (void)addScannerProDevicesToCloud:(NSArray <MKSDCreatScannerProDeviceModel *>*)deviceList
                             isHome:(BOOL)isHome
    ...
    for (MKSDCreatScannerProDeviceModel *deviceModel in deviceList) {
