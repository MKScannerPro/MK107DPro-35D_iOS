//
//  MKSDManageBleDevicesController.m
...
@interface MKSDManageBleDevicesController ()<UITableViewDelegate,
UITableViewDataSource,
MKSDManageBleDevicesSearchButtonDelegate,
MKSDManageBleDevicesCellDelegate>

@property (nonatomic, strong)MKBaseTableView *tableView;

@property (nonatomic, strong)MKSDManageBleDevicesSearchButtonModel *buttonModel;

@property (nonatomic, strong)MKSDManageBleDevicesSearchButton *searchButton;
...
@implementation MKSDManageBleDevicesController

- (void)dealloc {
    NSLog(@"MKSDManageBleDevicesController销毁");
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    MKSDManageBleDevicesCell *cell = [MKSDManageBleDevicesCell initCellWithTableView:tableView];
    cell.dataModel = self.dataList[indexPath.row];
    cell.delegate = self;
    return cell;
}

#pragma mark - MKSDManageBleDevicesSearchButtonDelegate
- (void)sd_scanSearchButtonMethod {
    [MKSDManageBleDeviceSearchView showSearchName:self.buttonModel.searchName
...
#pragma mark - MKSDManageBleDevicesCellDelegate
/// 用户点击了链接按钮
...
/*
 ...
 10：other
 */
- (void)sd_manageBleDevicesCellConnectButtonPressed:(NSString *)macAddress typeCode:(NSInteger)typeCode; {
    if (typeCode == 7) {
        //BXP-Button
        @weakify(self);
        ...
        [alertView showAlertWithTitle:@"Enter password" message:@"" notificationName:@"mk_sd_needDismissAlert"];
        return;
    }
    //Normal
...
- (void)addNotes {
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveDeviceDatas:)
                                                 name:MKSDReceiveDeviceDatasNotification
                                               object:nil];
}
...
- (void)updateCellModelWithMac:(NSString *)macAddress
    ...
    MKSDManageBleDevicesCellModel *cellModel = [[MKSDManageBleDevicesCellModel alloc] init];
...
- (void)connectBXPButtonWithPassword:(NSString *)password bleMac:(NSString *)bleMac {
    ...
    [MKSDMQTTInterface sd_connectBXPButtonWithPassword:password bleMac:bleMac macAddress:[MKSDDeviceModeManager shared].macAddress topic:[MKSDDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)connectNormalDeviceWithBleMac:(NSString *)macAddress {
    [[MKHudManager share] showHUDWithTitle:@"Connecting..." inView:self.view isPenetration:NO];
    [MKSDMQTTInterface sd_connectNormalBleDeviceWithBleMac:macAddress macAddress:[MKSDDeviceModeManager shared].macAddress topic:[MKSDDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        if ([returnData[@"data"][@"result_code"] integerValue] != 0) {
            //连接失败
            [self.view showCentralToast:returnData[@"data"][@"result_msg"]];
...
- (MKSDManageBleDevicesSearchButton *)searchButton {
    if (!_searchButton) {
        _searchButton = [[MKSDManageBleDevicesSearchButton alloc] init];
        _searchButton.delegate = self;
    }
    return _searchButton;
}

- (MKSDManageBleDevicesSearchButtonModel *)buttonModel {
    if (!_buttonModel) {
        _buttonModel = [[MKSDManageBleDevicesSearchButtonModel alloc] init];
