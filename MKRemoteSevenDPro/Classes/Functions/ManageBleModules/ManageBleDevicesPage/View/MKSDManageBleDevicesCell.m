...
@implementation MKSDManageBleDevicesCellModel
@end

@interface MKSDManageBleDevicesCell ()
...
@implementation MKSDManageBleDevicesCell

+ (MKSDManageBleDevicesCell *)initCellWithTableView:(UITableView *)tableView {
    MKSDManageBleDevicesCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKSDManageBleDevicesCellIdenty"];
    if (!cell) {
        cell = [[MKSDManageBleDevicesCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKSDManageBleDevicesCellIdenty"];
    }
    return cell;
}
...
- (void)connectButtonPressed {
    if ([self.delegate respondsToSelector:@selector(sd_manageBleDevicesCellConnectButtonPressed:typeCode:)]) {
        [self.delegate sd_manageBleDevicesCellConnectButtonPressed:SafeStr(self.dataModel.macAddress)
                                                          typeCode:self.dataModel.typeCode];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKSDManageBleDevicesCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKSDManageBleDevicesCellModel.class]) {
        return;
    }
