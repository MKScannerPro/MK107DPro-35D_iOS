//
//  MKSDNormalConnectedController.m
...
@interface MKSDNormalConnectedController ()<UITableViewDelegate,
UITableViewDataSource,
MKSDNormalConnectedCellDelegate>
...
@property (nonatomic, strong)MKSDConnectedDeviceWriteAlertView *writeAlert;

@end

@implementation MKSDNormalConnectedController

- (void)dealloc {
    NSLog(@"MKSDNormalConnectedController销毁");
...
- (void)leftButtonMethod {
    //用户点击左上角，则需要返回MKSDDeviceDataController
    [self popToViewControllerWithClassName:@"MKSDDeviceDataController"];
}
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    MKSDNormalConnectedCellModel *cellModel = charList[indexPath.row];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    MKSDNormalConnectedCell *cell = [MKSDNormalConnectedCell initCellWithTableView:tableView];
    NSDictionary *dic = self.sectionList[indexPath.section];
    NSArray *charList = dic[@"charList"];
...
#pragma mark - MKSDNormalConnectedCellDelegate
- (void)sd_normalConnectedCell_writeButtonPressed:(NSInteger)section
                                              row:(NSInteger)row
                                       serverUUID:(NSString *)serverUUID
                                   characteristic:(NSString *)characteristic {
    @weakify(self);
    [self.writeAlert showAlertWithValue:@"" dismissNote:@"mk_sd_needDismissAlert" cancelAction:^{
...
- (void)sd_normalConnectedCell_readButtonPressed:(NSInteger)section
                                             row:(NSInteger)row
                                      serverUUID:(NSString *)serverUUID
                                  characteristic:(NSString *)characteristic {
...
- (void)sd_normalConnectedCell_notifyButtonPressed:(BOOL)notify
...
- (void)receiveDisconnect:(NSNotification *)note {
    ...
    if (!ValidDict(user) || !ValidStr(user[@"device_info"][@"mac"]) || ![[MKSDDeviceModeManager shared].macAddress isEqualToString:user[@"device_info"][@"mac"]]) {
        return;
    }
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_sd_needDismissAlert" object:nil];
    //返回上一级页面
...
- (void)receiveDeviceDatas:(NSNotification *)note {
    ...
    MKSDNormalConnectedCellModel *cellModel = charList[row];
...
- (void)addNotes {
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveDisconnect:)
                                                 name:MKSDReceiveGatewayDisconnectDeviceNotification
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveDeviceDatas:)
                                                 name:MKSDReceiveGatewayConnectedDeviceDatasNotification
                                               object:nil];
}
...
- (void)parseTableDatas {
    ...
    for (NSInteger i = 0; i < count; i ++) {
        ...
        for (NSInteger j = 0; j < charCount; j ++) {
            ...
            MKSDNormalConnectedCellModel *cellModel = [[MKSDNormalConnectedCellModel alloc] init];
            cellModel.section = i;
            cellModel.row = j;
...
- (void)disconnect {
    ...
    [MKSDMQTTInterface sd_disconnectNormalBleDeviceWithBleMac:self.deviceBleInfo[@"data"][@"mac"] macAddress:[MKSDDeviceModeManager shared].macAddress topic:[MKSDDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)writeData:(NSString *)data serviceUUID:(NSString *)serverUUID characteristic:(NSString *)characteristic {
    ...
    [MKSDMQTTInterface sd_writeValueToDeviceWithBleMacAddress:self.deviceBleInfo[@"data"][@"mac"] value:data serviceUUID:serverUUID characteristicUUID:characteristic macAddress:[MKSDDeviceModeManager shared].macAddress topic:[MKSDDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
...
- (void)readDataWithServiceUUID:(NSString *)serverUUID
    ...
    [MKSDMQTTInterface sd_readCharacteristicValueWithBleMacAddress:self.deviceBleInfo[@"data"][@"mac"] serviceUUID:serverUUID characteristicUUID:characteristic macAddress:[MKSDDeviceModeManager shared].macAddress topic:[MKSDDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        if ([returnData[@"data"][@"result_code"] integerValue] != 0) {
...
characteristic:(NSString *)characteristic
    ...
    [MKSDMQTTInterface sd_notifyCharacteristic:notify bleMacAddress:self.deviceBleInfo[@"data"][@"mac"] serviceUUID:serverUUID characteristicUUID:characteristic macAddress:[MKSDDeviceModeManager shared].macAddress topic:[MKSDDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        if ([returnData[@"data"][@"result_code"] integerValue] != 0) {
            [self.view showCentralToast:@"Read Failed"];
            return;
...
#pragma mark - UI
- (void)loadSubViews {
    self.defaultTitle = [MKSDDeviceModeManager shared].deviceName;
...
- (MKSDConnectedDeviceWriteAlertView *)writeAlert {
    if (!_writeAlert) {
        _writeAlert = [[MKSDConnectedDeviceWriteAlertView alloc] init];
    }
    return _writeAlert;
}
