...
@interface MKSDButtonDFUController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKSDButtonDFUModel *dataModel;
...
@implementation MKSDButtonDFUController

- (void)dealloc {
    NSLog(@"MKSDButtonDFUController销毁");
...
- (void)addNotes {
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveDfuProgress:)
                                                 name:MKSDReceiveBxpButtonDfuProgressNotification
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveDfuResult:)
                                                 name:MKSDReceiveBxpButtonDfuResultNotification
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
...
- (void)gotoLastPage {
    ...
    for (UIViewController *vc in self.navigationController.viewControllers) {
        if ([vc isKindOfClass:NSClassFromString(@"MKSDManageBleDevicesController")]) {
            //如果是从Manage BLE devices页面进来的
            [self popToViewControllerWithClassName:@"MKSDManageBleDevicesController"];
            break;
        }
        if ([vc isKindOfClass:NSClassFromString(@"MKSDDeviceDataController")]) {
            //如果是从扫描页面进来的
...
- (MKSDButtonDFUModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKSDButtonDFUModel alloc] init];
