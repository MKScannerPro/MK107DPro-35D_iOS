...
@implementation MKSDButtonFirmwareCellModel
@end

@interface MKSDButtonFirmwareCell ()
...
@implementation MKSDButtonFirmwareCell

+ (MKSDButtonFirmwareCell *)initCellWithTableView:(UITableView *)tableView {
    MKSDButtonFirmwareCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKSDButtonFirmwareCellIdenty"];
    if (!cell) {
        cell = [[MKSDButtonFirmwareCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKSDButtonFirmwareCellIdenty"];
    }
    return cell;
}
...
- (void)rightButtonPressed {
    if ([self.delegate respondsToSelector:@selector(sd_buttonFirmwareCell_buttonAction:)]) {
        [self.delegate sd_buttonFirmwareCell_buttonAction:self.dataModel.index];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKSDButtonFirmwareCellModel *)dataModel {
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKSDButtonFirmwareCellModel.class]) {
        return;
    }
