...
@interface MKSDBXPButtonController ()<UITableViewDelegate,
UITableViewDataSource,
MKButtonMsgCellDelegate,
MKSDButtonFirmwareCellDelegate>
...
@implementation MKSDBXPButtonController

- (void)dealloc {
    NSLog(@"MKSDBXPButtonController销毁");
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKSDButtonFirmwareCell *cell = [MKSDButtonFirmwareCell initCellWithTableView:tableView];
...
#pragma mark - MKSDButtonFirmwareCellDelegate
- (void)sd_buttonFirmwareCell_buttonAction:(NSInteger)index {
    if (index == 0) {
        //DFU
        MKSDButtonDFUController *vc = [[MKSDButtonDFUController alloc] init];
...
- (void)readDatasFromDevice {
    ...
    [MKSDMQTTInterface sd_readBXPButtonConnectedStatusWithBleMacAddress:self.deviceBleInfo[@"data"][@"mac"] macAddress:[MKSDDeviceModeManager shared].macAddress topic:[MKSDDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)dismissAlarmStatus {
    ...
    [MKSDMQTTInterface sd_dismissBXPButtonAlarmStatusWithBleMacAddress:self.deviceBleInfo[@"data"][@"mac"] macAddress:[MKSDDeviceModeManager shared].macAddress topic:[MKSDDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        if ([returnData[@"data"][@"result_code"] integerValue] != 0) {
            [self.view showCentralToast:@"Read Failed"];
            return;
...
- (void)sendLedReminder:(NSString *)interval duration:(NSString *)duration color:(NSInteger)color {
    ...
    [MKSDMQTTInterface sd_configDeviceLedReminderWithBleMac:self.deviceBleInfo[@"data"][@"mac"] interval:[interval integerValue] duration:[duration integerValue] macAddress:[MKSDDeviceModeManager shared].macAddress topic:[MKSDDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        if ([returnData[@"data"][@"result_code"] integerValue] != 0) {
...
- (void)sendBuzzerReminder:(NSString *)interval duration:(NSString *)duration {
    ...
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKSDMQTTInterface sd_configDeviceBuzzerReminderWithBleMac:self.deviceBleInfo[@"data"][@"mac"] interval:[interval integerValue] duration:[duration integerValue] macAddress:[MKSDDeviceModeManager shared].macAddress topic:[MKSDDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        if ([returnData[@"data"][@"result_code"] integerValue] != 0) {
            [self.view showCentralToast:@"setup failed!"];
            return;
...
- (void)addNotes {
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveDisconnect:)
                                                 name:MKSDReceiveGatewayDisconnectBXPButtonNotification
                                               object:nil];
}

- (void)showLedReminderAlert {
    MKSDReminderAlertViewModel *dataModel = [[MKSDReminderAlertViewModel alloc] init];
    dataModel.title = @"LED Reminder";
    dataModel.intervalMsg = @"Blinking interval";
    dataModel.durationMsg = @"Blinking duration";
    MKSDReminderAlertView *alertView = [[MKSDReminderAlertView alloc] init];
...
- (void)loadSection1Datas {
    MKSDButtonFirmwareCellModel *cellModel = [[MKSDButtonFirmwareCellModel alloc] init];
    cellModel.index = 0;
