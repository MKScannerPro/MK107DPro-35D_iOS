...
@interface MKSDBleAdvBeaconController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKSDBleAdvBeaconModel *dataModel;

@end

@implementation MKSDBleAdvBeaconController

- (void)dealloc {
    NSLog(@"MKSDBleAdvBeaconController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKRemoteSevenDPro", @"MKSDBleAdvBeaconController", @"sd_saveIcon.png") forState:UIControlStateNormal];
...
- (MKSDBleAdvBeaconModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKSDBleAdvBeaconModel alloc] init];
    }
    return _dataModel;
}
