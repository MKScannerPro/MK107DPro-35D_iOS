//
//  MKSDServerConfigDeviceSettingView.m
...
@interface MKSDServerConfigDeviceSettingView ()
...
@implementation MKSDServerConfigDeviceSettingView
...
- (void)clearButtonPressed {
    if ([self.delegate respondsToSelector:@selector(sd_mqtt_deviecSetting_fileButtonPressed:)]) {
        [self.delegate sd_mqtt_deviecSetting_fileButtonPressed:2];
    }
}

- (void)exportButtonPressed {
    if ([self.delegate respondsToSelector:@selector(sd_mqtt_deviecSetting_fileButtonPressed:)]) {
        [self.delegate sd_mqtt_deviecSetting_fileButtonPressed:0];
    }
}

- (void)importButtonPressed {
    if ([self.delegate respondsToSelector:@selector(sd_mqtt_deviecSetting_fileButtonPressed:)]) {
        [self.delegate sd_mqtt_deviecSetting_fileButtonPressed:1];
    }
}
