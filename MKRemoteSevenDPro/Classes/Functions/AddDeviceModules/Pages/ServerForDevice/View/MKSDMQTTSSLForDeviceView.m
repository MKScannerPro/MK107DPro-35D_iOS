...
@implementation MKSDMQTTSSLForDeviceViewModel
@end

@interface MKSDMQTTSSLForDeviceView ()<MKMQTTSSLCertificateViewDelegate>
...
@implementation MKSDMQTTSSLForDeviceView
...
- (void)mk_fileSelectedButtonPressed:(NSInteger)index {
    if (index == 0) {
        //CA File
        if ([self.delegate respondsToSelector:@selector(sd_mqtt_sslParams_device_caFilePressed)]) {
            [self.delegate sd_mqtt_sslParams_device_caFilePressed];
        }
        return;
    }
    if (index == 1) {
        //Client Key File
        if ([self.delegate respondsToSelector:@selector(sd_mqtt_sslParams_device_clientKeyPressed)]) {
            [self.delegate sd_mqtt_sslParams_device_clientKeyPressed];
        }
        return;
    }
    //Client Cert File
    if ([self.delegate respondsToSelector:@selector(sd_mqtt_sslParams_device_clientCertPressed)]) {
        [self.delegate sd_mqtt_sslParams_device_clientCertPressed];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(sd_mqtt_sslParams_device_sslStatusChanged:)]) {
        [self.delegate sd_mqtt_sslParams_device_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(sd_mqtt_sslParams_device_certificateChanged:)]) {
            [self.delegate sd_mqtt_sslParams_device_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKSDMQTTSSLForDeviceViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKSDMQTTSSLForDeviceViewModel.class]) {
        return;
    }
...
- (void)updateSSLButtonIcon {
    UIImage *image = (self.sslButton.selected ? LOADICON(@"MKRemoteSevenDPro", @"MKSDMQTTSSLForDeviceView", @"sd_switchSelectedIcon.png") : LOADICON(@"MKRemoteSevenDPro", @"MKSDMQTTSSLForDeviceView", @"sd_switchUnselectedIcon.png"));
...
- (UIButton *)sslButton {
    if (!_sslButton) {
        ...
        [_sslButton setImage:LOADICON(@"MKRemoteSevenDPro", @"MKSDMQTTSSLForDeviceView", @"sd_switchUnselectedIcon.png") forState:UIControlStateNormal];
