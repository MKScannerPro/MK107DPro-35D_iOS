//
//  MKSDServerConfigDeviceFooterView.m
...
@implementation MKSDServerConfigDeviceFooterViewModel
@end
...
@interface MKSDServerConfigDeviceFooterView ()<UIScrollViewDelegate,
MKMQTTGeneralParamsViewDelegate,
MKMQTTUserCredentialsViewDelegate,
MKSDMQTTSSLForDeviceViewDelegate,
MKSDServerConfigDeviceSettingViewDelegate,
MKSDMQTTLWTForDeviceViewDelegate>
...
@property (nonatomic, strong)MKSDMQTTSSLForDeviceView *sslView;

@property (nonatomic, strong)MKSDMQTTLWTForDeviceView *lwtView;

@property (nonatomic, strong)MKSDServerConfigDeviceSettingView *settingView;

@property (nonatomic, assign)NSInteger index;

@end

@implementation MKSDServerConfigDeviceFooterView
...
- (void)mk_mqtt_generalParams_cleanSessionStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(sd_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
        [self.delegate sd_mqtt_serverForDevice_switchStatusChanged:isOn statusID:0];
    }
}

- (void)mk_mqtt_generalParams_qosChanged:(NSInteger)qos {
    if ([self.delegate respondsToSelector:@selector(sd_mqtt_serverForDevice_qosChanged:qosID:)]) {
        [self.delegate sd_mqtt_serverForDevice_qosChanged:qos qosID:0];
    }
}

- (void)mk_mqtt_generalParams_KeepAliveChanged:(NSString *)keepAlive {
    if ([self.delegate respondsToSelector:@selector(sd_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate sd_mqtt_serverForDevice_textFieldValueChanged:keepAlive textID:0];
    }
}
...
- (void)mk_mqtt_userCredentials_userNameChanged:(NSString *)userName {
    if ([self.delegate respondsToSelector:@selector(sd_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate sd_mqtt_serverForDevice_textFieldValueChanged:userName textID:1];
    }
}

- (void)mk_mqtt_userCredentials_passwordChanged:(NSString *)password {
    if ([self.delegate respondsToSelector:@selector(sd_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate sd_mqtt_serverForDevice_textFieldValueChanged:password textID:2];
    }
}

#pragma mark - MKSDMQTTSSLForDeviceViewDelegate
- (void)sd_mqtt_sslParams_device_sslStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(sd_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
        [self.delegate sd_mqtt_serverForDevice_switchStatusChanged:isOn statusID:1];
    }
}
...
- (void)sd_mqtt_sslParams_device_certificateChanged:(NSInteger)certificate {
    if ([self.delegate respondsToSelector:@selector(sd_mqtt_serverForDevice_certificateChanged:)]) {
        [self.delegate sd_mqtt_serverForDevice_certificateChanged:certificate];
    }
}

/// 用户点击选择了caFaile按钮
- (void)sd_mqtt_sslParams_device_caFilePressed {
    if ([self.delegate respondsToSelector:@selector(sd_mqtt_serverForDevice_fileButtonPressed:)]) {
        [self.delegate sd_mqtt_serverForDevice_fileButtonPressed:0];
    }
}
...
- (void)sd_mqtt_sslParams_device_clientKeyPressed {
    if ([self.delegate respondsToSelector:@selector(sd_mqtt_serverForDevice_fileButtonPressed:)]) {
        [self.delegate sd_mqtt_serverForDevice_fileButtonPressed:1];
    }
}
...
- (void)sd_mqtt_sslParams_device_clientCertPressed {
    if ([self.delegate respondsToSelector:@selector(sd_mqtt_serverForDevice_fileButtonPressed:)]) {
        [self.delegate sd_mqtt_serverForDevice_fileButtonPressed:2];
    }
}

#pragma mark - MKSDServerConfigDeviceSettingViewDelegate

/// 底部按钮
/// @param index 0:Export Demo File   1:Import Config File
- (void)sd_mqtt_deviecSetting_fileButtonPressed:(NSInteger)index {
    if ([self.delegate respondsToSelector:@selector(sd_mqtt_serverForDevice_bottomButtonPressed:)]) {
        [self.delegate sd_mqtt_serverForDevice_bottomButtonPressed:index];
    }
}

#pragma mark - MKSDMQTTLWTForDeviceViewDelegate
- (void)sd_lwt_statusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(sd_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
        [self.delegate sd_mqtt_serverForDevice_switchStatusChanged:isOn statusID:2];
    }
}

- (void)sd_lwt_retainChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(sd_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
        [self.delegate sd_mqtt_serverForDevice_switchStatusChanged:isOn statusID:3];
    }
}

- (void)sd_lwt_qosChanged:(NSInteger)qos {
    if ([self.delegate respondsToSelector:@selector(sd_mqtt_serverForDevice_qosChanged:qosID:)]) {
        [self.delegate sd_mqtt_serverForDevice_qosChanged:qos qosID:1];
    }
}

- (void)sd_lwt_topicChanged:(NSString *)text {
    if ([self.delegate respondsToSelector:@selector(sd_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate sd_mqtt_serverForDevice_textFieldValueChanged:text textID:5];
    }
}

- (void)sd_lwt_payloadChanged:(NSString *)text {
    if ([self.delegate respondsToSelector:@selector(sd_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate sd_mqtt_serverForDevice_textFieldValueChanged:text textID:6];
    }
}
...
#pragma mark - setter
- (void)setDataModel:(MKSDServerConfigDeviceFooterViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKSDServerConfigDeviceFooterViewModel.class]) {
        return;
    }
    ...
    MKSDMQTTSSLForDeviceViewModel *sslModel = [[MKSDMQTTSSLForDeviceViewModel alloc] init];
    ...
    MKSDMQTTLWTForDeviceViewModel *lwtModel = [[MKSDMQTTLWTForDeviceViewModel alloc] init];
...
- (MKSDMQTTSSLForDeviceView *)sslView {
    if (!_sslView) {
        _sslView = [[MKSDMQTTSSLForDeviceView alloc] init];
...
- (MKSDMQTTLWTForDeviceView *)lwtView {
    if (!_lwtView) {
        _lwtView = [[MKSDMQTTLWTForDeviceView alloc] init];
...
- (MKSDServerConfigDeviceSettingView *)settingView {
    if (!_settingView) {
        _settingView = [[MKSDServerConfigDeviceSettingView alloc] init];
