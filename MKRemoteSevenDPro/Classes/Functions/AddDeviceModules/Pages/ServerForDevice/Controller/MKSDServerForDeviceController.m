//
//  MKSDServerForDeviceController.m
...
@interface MKSDServerForDeviceController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
MKSDServerConfigDeviceFooterViewDelegate,
MKCAFileSelectControllerDelegate,
MFMailComposeViewControllerDelegate,
MKSDImportServerControllerDelegate>
...
@property (nonatomic, strong)MKSDServerForDeviceModel *dataModel;

@property (nonatomic, strong)MKSDServerConfigDeviceFooterView *sslParamsView;

@property (nonatomic, strong)MKSDServerConfigDeviceFooterViewModel *sslParamsModel;

@property (nonatomic, strong)UIView *footerView;

@end

@implementation MKSDServerForDeviceController

- (void)dealloc {
    NSLog(@"MKSDServerForDeviceController销毁");
}
...
#pragma mark - MKSDServerConfigDeviceFooterViewDelegate
/// 用户改变了开关状态
/// @param isOn isOn
/// @param statusID 0:cleanSession   1:ssl    2:lwtStatus  3:lwtRetain
- (void)sd_mqtt_serverForDevice_switchStatusChanged:(BOOL)isOn statusID:(NSInteger)statusID {
    if (statusID == 0) {
        //cleanSession
...
- (void)sd_mqtt_serverForDevice_textFieldValueChanged:(NSString *)text textID:(NSInteger)textID {
    if (textID == 0) {
        //keepAlive
...
- (void)sd_mqtt_serverForDevice_qosChanged:(NSInteger)qos qosID:(NSInteger)qosID{
    if (qosID == 0) {
        //qos
...
- (void)sd_mqtt_serverForDevice_certificateChanged:(NSInteger)certificate {
...
- (void)sd_mqtt_serverForDevice_fileButtonPressed:(NSInteger)fileType {
    if (fileType == 0) {
        //caFile
...
- (void)sd_mqtt_serverForDevice_bottomButtonPressed:(NSInteger)index {
    if (index == 0) {
        //Export Demo File
    ...
    if (index == 1) {
        //Import Config File
        MKSDImportServerController *vc = [[MKSDImportServerController alloc] init];
        vc.delegate = self;
        [self.navigationController pushViewController:vc animated:YES];
    }
    if (index == 2) {
...
#pragma mark - MKSDImportServerControllerDelegate
- (void)sd_selectedServerParams:(NSString *)fileName {
    [MKSDExcelDataManager parseDeviceExcel:fileName
                                   sucBlock:^(NSDictionary * _Nonnull returnData) {
        MKSDServerForDeviceModel *model = [MKSDServerForDeviceModel mk_modelWithJSON:returnData];
...
- (void)saveDataToDevice {
    ...
    [self.dataModel configDataWithSucBlock:^{
        ...
        [MKSDDeviceMQTTParamsModel shared].mqttConfig = YES;
        [MKSDDeviceMQTTParamsModel shared].deviceModel.clientID = self.dataModel.clientID;
        [MKSDDeviceMQTTParamsModel shared].deviceModel.deviceName = self.dataModel.deviceName;
        [MKSDDeviceMQTTParamsModel shared].deviceModel.subscribedTopic = self.dataModel.subscribeTopic;
        [MKSDDeviceMQTTParamsModel shared].deviceModel.publishedTopic = self.dataModel.publishTopic;
        [MKSDDeviceMQTTParamsModel shared].deviceModel.macAddress = self.dataModel.macAddress;
        [MKSDDeviceMQTTParamsModel shared].deviceModel.lwtStatus = self.dataModel.lwtStatus;
        [MKSDDeviceMQTTParamsModel shared].deviceModel.lwtTopic = self.dataModel.lwtTopic;
    } failedBlock:^(NSError * _Nonnull error) {
        @strongify(self);
        [[MKHudManager share] hide];
        [self.view showCentralToast:error.userInfo[@"errorInfo"]];
        [MKSDDeviceMQTTParamsModel shared].deviceModel.clientID = @"";
        [MKSDDeviceMQTTParamsModel shared].deviceModel.deviceName = @"";
        [MKSDDeviceMQTTParamsModel shared].deviceModel.subscribedTopic = @"";
        [MKSDDeviceMQTTParamsModel shared].deviceModel.publishedTopic = @"";
        [MKSDDeviceMQTTParamsModel shared].deviceModel.macAddress = @"";
        [MKSDDeviceMQTTParamsModel shared].deviceModel.lwtStatus = NO;
        [MKSDDeviceMQTTParamsModel shared].deviceModel.lwtTopic = @"";
    }];
}
...
//    }
    ...
    [MKSDExcelDataManager exportDeviceExcel:self.dataModel
                                    sucBlock:^{
        [[MKHudManager share] hide];
        [self sharedExcel];
    }
...
- (void)showClearAlertView {
    ...
    [alertView showAlertWithTitle:@"" message:msg notificationName:@"mk_sd_needDismissAlert"];
}

- (void)clearAllParams {
    MKSDServerForDeviceModel *emptyModel = [[MKSDServerForDeviceModel alloc] init];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKRemoteSevenDPro", @"MKSDServerForDeviceController", @"sd_saveIcon.png") forState:UIControlStateNormal];
...
- (MKSDServerForDeviceModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKSDServerForDeviceModel alloc] init];
    }
    return _dataModel;
}
...
- (MKSDServerConfigDeviceFooterView *)sslParamsView {
    if (!_sslParamsView) {
        _sslParamsView = [[MKSDServerConfigDeviceFooterView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 380.f)];
...
- (MKSDServerConfigDeviceFooterViewModel *)sslParamsModel {
    if (!_sslParamsModel) {
        _sslParamsModel = [[MKSDServerConfigDeviceFooterViewModel alloc] init];
    }
