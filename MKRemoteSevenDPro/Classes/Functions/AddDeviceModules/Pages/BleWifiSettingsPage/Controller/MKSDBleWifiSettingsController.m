//
//  MKSDBleWifiSettingsController.m
...
#include "MKSDBleWifiSettingsCertCell.h"
#include "MKSDBleWifiSettingsBandCell.h"
...
@interface MKSDBleWifiSettingsController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextButtonCellDelegate,
MKTextFieldCellDelegate,
mk_textSwitchCellDelegate,
MKSDBleWifiSettingsCertCellDelegate,
MKCAFileSelectControllerDelegate,
MKSDBleWifiSettingsBandCellDelegate>
...
@property (nonatomic, strong)MKSDBleWifiSettingsModel *dataModel;

@end

@implementation MKSDBleWifiSettingsController

- (void)dealloc {
    NSLog(@"MKSDBleWifiSettingsController销毁");
}
...
#pragma mark - MKSDBleWifiSettingsCertCellDelegate
...
- (void)mk_certSelectedMethod:(mk_certListPageType)certType certName:(NSString *)certName {
    if (certType == mk_caCertSelPage) {
        ...
        MKSDBleWifiSettingsCertCellModel *cellModel = self.section8List[0];
    ...
    if (certType == mk_clientCertSelPage) {
        ...
        MKSDBleWifiSettingsCertCellModel *cellModel = self.section9List[0];
        cellModel.fileName = certName;
    ...
    if (certType == mk_clientKeySelPage) {
        ...
        MKSDBleWifiSettingsCertCellModel *cellModel = self.section10List[0];
        cellModel.fileName = certName;
...
#pragma mark - MKSDBleWifiSettingsBandCellDelegate
- (void)sd_bleWifiSettingsBandCell_countryChanged:(NSInteger)country {
    MKSDBleWifiSettingsBandCellModel *cellModel = self.section11List[0];
...
- (void)saveDataToDevice {
    ...
    [self.dataModel configDataWithSucBlock:^{
        ...
        [MKSDDeviceMQTTParamsModel shared].wifiConfig = YES;
    } failedBlock:^(NSError * _Nonnull error) {
        @strongify(self);
        [[MKHudManager share] hide];
        [self.view showCentralToast:error.userInfo[@"errorInfo"]];
        [MKSDDeviceMQTTParamsModel shared].wifiConfig = NO;
    }];
}
...
- (UITableViewCell *)cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 8) {
        //CA certificate.
        MKSDBleWifiSettingsCertCell *cell = [MKSDBleWifiSettingsCertCell initCellWithTableView:self.tableView];
    ...
    //Country&Band
    MKSDBleWifiSettingsBandCell *cell = [MKSDBleWifiSettingsBandCell initCellWithTableView:self.tableView];
...
- (void)loadSection8Datas {
    MKSDBleWifiSettingsCertCellModel *cellModel = [[MKSDBleWifiSettingsCertCellModel alloc] init];
    cellModel.index = 0;
...
- (void)loadSection11Datas {
    MKSDBleWifiSettingsBandCellModel *cellModel = [[MKSDBleWifiSettingsBandCellModel alloc] init];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKRemoteSevenDPro", @"MKSDBleWifiSettingsController", @"sd_saveIcon.png") forState:UIControlStateNormal];
...
- (MKSDBleWifiSettingsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKSDBleWifiSettingsModel alloc] init];
    }
    return _dataModel;
}
