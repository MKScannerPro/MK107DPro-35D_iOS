...
@implementation MKSDBleWifiSettingsBandCellModel
@end

@interface MKSDBleWifiSettingsBandCell ()
...
@implementation MKSDBleWifiSettingsBandCell

+ (MKSDBleWifiSettingsBandCell *)initCellWithTableView:(UITableView *)tableView {
    MKSDBleWifiSettingsBandCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKSDBleWifiSettingsBandCellIdenty"];
    if (!cell) {
        cell = [[MKSDBleWifiSettingsBandCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKSDBleWifiSettingsBandCellIdenty"];
    }
    return cell;
}
...
- (void)selectedButtonPressed {
    ...
    [pickView showPickViewWithDataList:self.dataList selectedRow:row block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(sd_bleWifiSettingsBandCell_countryChanged:)]) {
            [self.delegate sd_bleWifiSettingsBandCell_countryChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKSDBleWifiSettingsBandCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKSDBleWifiSettingsBandCellModel.class] || _dataModel.country >= self.dataList.count) {
        return;
    }
