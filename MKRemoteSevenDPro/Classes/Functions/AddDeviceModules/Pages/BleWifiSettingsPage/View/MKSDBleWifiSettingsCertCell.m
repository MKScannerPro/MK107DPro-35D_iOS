...
@implementation MKSDBleWifiSettingsCertCellModel
@end

@interface MKSDBleWifiSettingsCertCell ()
...
@implementation MKSDBleWifiSettingsCertCell

+ (MKSDBleWifiSettingsCertCell *)initCellWithTableView:(UITableView *)tableView {
    MKSDBleWifiSettingsCertCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKSDBleWifiSettingsCertCellIdenty"];
    if (!cell) {
        cell = [[MKSDBleWifiSettingsCertCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKSDBleWifiSettingsCertCellIdenty"];
    }
    return cell;
}
...
- (void)certificateButtonPressed {
    if ([self.delegate respondsToSelector:@selector(sd_bleWifiSettingsCertPressed:)]) {
        [self.delegate sd_bleWifiSettingsCertPressed:self.dataModel.index];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKSDBleWifiSettingsCertCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKSDBleWifiSettingsCertCellModel.class]) {
        return;
    }
...
- (UIButton *)certificateButton {
    if (!_certificateButton) {
        ...
        [_certificateButton setImage:LOADICON(@"MKRemoteSevenDPro", @"MKSDBleWifiSettingsCertCell", @"sd_config_certAddIcon.png") forState:UIControlStateNormal];
