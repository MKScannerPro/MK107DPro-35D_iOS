//
//  MKSDBleNTPTimezoneController.m
...
@interface MKSDBleNTPTimezoneController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKSDBleNTPTimezoneModel *dataModel;

@property (nonatomic, strong)NSMutableArray *timezoneList;

@end

@implementation MKSDBleNTPTimezoneController

- (void)dealloc {
    NSLog(@"MKSDBleNTPTimezoneController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKRemoteSevenDPro", @"MKSDBleNTPTimezoneController", @"sd_saveIcon.png") forState:UIControlStateNormal];
...
- (MKSDBleNTPTimezoneModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKSDBleNTPTimezoneModel alloc] init];
    }
    return _dataModel;
}
