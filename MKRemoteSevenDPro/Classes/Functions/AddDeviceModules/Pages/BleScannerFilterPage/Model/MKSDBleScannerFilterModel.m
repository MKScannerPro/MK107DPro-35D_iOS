...
@interface MKSDBleScannerFilterModel ()
...
@implementation MKSDBleScannerFilterModel
...
- (BOOL)readFilterRssi {
    ...
    [MKSDInterface sd_readRssiFilterValueWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterRssi {
    __block BOOL success = NO;
    [MKSDInterface sd_configRssiFilterValue:self.rssi sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readMacAddress {
    __block BOOL success = NO;
    [MKSDInterface sd_readFilterMACAddressListWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configMacAddress {
    ...
    [MKSDInterface sd_configFilterMACAddressList:macList
                             sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    }
...
- (BOOL)readAdvName {
    __block BOOL success = NO;
    [MKSDInterface sd_readFilterAdvNameListWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configAdvName {
    ...
    [MKSDInterface sd_configFilterAdvNameList:nameList
                             sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    }
...
- (BOOL)configRelationShip {
    __block BOOL success = NO;
    [MKSDInterface sd_configFilterRelationship:7
                             sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    }
