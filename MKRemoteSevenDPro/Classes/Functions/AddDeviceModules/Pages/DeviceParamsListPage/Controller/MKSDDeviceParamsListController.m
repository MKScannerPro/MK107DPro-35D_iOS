//
//  MKSDDeviceParamsListController.m
...
@interface MKSDDeviceParamsListController ()<UITableViewDelegate, UITableViewDataSource>
...
@implementation MKSDDeviceParamsListController

- (void)dealloc {
    NSLog(@"MKSDDeviceParamsListController销毁");
...
- (void)viewDidLoad {
    ...
    [MKSDDeviceMQTTParamsModel shared].deviceModel.deviceType = self.deviceType;
...
- (void)leftButtonMethod {
    ...
    [[MKSDCentralManager shared] disconnect];
    [MKSDDeviceMQTTParamsModel sharedDealloc];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0) {
        //WIFI Settings
        MKSDBleWifiSettingsController *vc = [[MKSDBleWifiSettingsController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 1) {
        //MQTT settings
        MKSDServerForDeviceController *vc = [[MKSDServerForDeviceController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 1 && indexPath.row == 0) {
        //Network Settings
        MKSDBleNetworkSettingsController *vc = [[MKSDBleNetworkSettingsController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 1 && indexPath.row == 1) {
        //NTP & Timezone
        MKSDBleNTPTimezoneController *vc = [[MKSDBleNTPTimezoneController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 1 && indexPath.row == 2) {
        //Scanner Filter
        MKSDBleScannerFilterController *vc = [[MKSDBleScannerFilterController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 1 && indexPath.row == 3) {
        //Advertise iBeacon
        MKSDBleAdvBeaconController *vc = [[MKSDBleAdvBeaconController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 1 && indexPath.row == 4) {
        //Device Information
        MKSDBleDeviceInfoController *vc = [[MKSDBleDeviceInfoController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
}
...
- (void)receiveDeviceOnline:(NSNotification *)note {
    ...
    if (!ValidDict(user) || !ValidStr(user[@"macAddress"]) || ![[MKSDDeviceMQTTParamsModel shared].deviceModel.macAddress isEqualToString:user[@"macAddress"]]) {
        return;
    }
...
- (void)connectButtonPressed {
    if (![MKSDDeviceMQTTParamsModel shared].wifiConfig || ![MKSDDeviceMQTTParamsModel shared].mqttConfig) {
        [self.view showCentralToast:@"Please configure WIFI and MQTT settings first!"];
        return;
    }
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKSDInterface sd_enterSTAModeWithSucBlock:^{
...
- (void)startMqttProcess {
    ...
    if (ValidStr([MKSDMQTTDataManager shared].serverParams.subscribeTopic)) {
        //查看是否设置了服务器的订阅topic
        topic = [MKSDMQTTDataManager shared].serverParams.subscribeTopic;
    }else {
        topic = [MKSDDeviceMQTTParamsModel shared].deviceModel.publishedTopic;
    }
    [self.progressView show];
    [[MKSDMQTTDataManager shared] subscriptions:@[topic]];
    [[NSNotificationCenter defaultCenter] addObserver:self
...
- (void)connectSuccess {
    ...
    MKSDDeviceModel *deviceModel = [[MKSDDeviceModel alloc] init];
    deviceModel.deviceType = [MKSDDeviceMQTTParamsModel shared].deviceModel.deviceType;
    deviceModel.clientID = [MKSDDeviceMQTTParamsModel shared].deviceModel.clientID;
    deviceModel.subscribedTopic = [MKSDDeviceMQTTParamsModel shared].deviceModel.subscribedTopic;
    deviceModel.publishedTopic = [MKSDDeviceMQTTParamsModel shared].deviceModel.publishedTopic;
    deviceModel.macAddress = [MKSDDeviceMQTTParamsModel shared].deviceModel.macAddress;
    deviceModel.deviceName = [MKSDDeviceMQTTParamsModel shared].deviceModel.deviceName;
    deviceModel.lwtStatus = [MKSDDeviceMQTTParamsModel shared].deviceModel.lwtStatus;
    deviceModel.lwtTopic = [MKSDDeviceMQTTParamsModel shared].deviceModel.lwtTopic;
        
    MKSDConnectSuccessController *vc = [[MKSDConnectSuccessController alloc] init];
