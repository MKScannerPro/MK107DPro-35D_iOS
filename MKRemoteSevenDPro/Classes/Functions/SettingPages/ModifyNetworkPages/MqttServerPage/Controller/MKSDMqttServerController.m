...
@interface MKSDMqttServerController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
MKSDMqttServerConfigFooterViewDelegate,
...
@property (nonatomic, strong)MKSDMqttServerModel *dataModel;

@property (nonatomic, strong)MKSDMqttServerConfigFooterView *sslParamsView;

@property (nonatomic, strong)MKSDMqttServerConfigFooterViewModel *sslParamsModel;

@property (nonatomic, strong)UIView *footerView;

@end

@implementation MKSDMqttServerController

- (void)dealloc {
    NSLog(@"MKSDMqttServerController销毁");
}
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(receiveUpdateCerts:)
                                                 name:MKSDReceiveDeviceUpdateMqttCertsResultNotification
                                               object:nil];
}
...
#pragma mark - MKSDMqttServerConfigFooterViewDelegate
/// 用户改变了开关状态
/// @param isOn isOn
...
- (void)sd_selectedServerParams:(NSString *)fileName {
    [MKSDExcelDataManager parseDeviceExcel:fileName
        ...
        MKSDMqttServerModel *model = [MKSDMqttServerModel mk_modelWithJSON:returnData];
...
- (void)clearAllParams {
    MKSDMqttServerModel *model = [[MKSDMqttServerModel alloc] init];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKRemoteSevenDPro", @"MKSDMqttServerController", @"sd_saveIcon.png") forState:UIControlStateNormal];
...
- (MKSDMqttServerModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKSDMqttServerModel alloc] init];
    }
    return _dataModel;
}
...
- (MKSDMqttServerConfigFooterView *)sslParamsView {
    if (!_sslParamsView) {
        _sslParamsView = [[MKSDMqttServerConfigFooterView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 380.f)];
...
- (MKSDMqttServerConfigFooterViewModel *)sslParamsModel {
    if (!_sslParamsModel) {
        _sslParamsModel = [[MKSDMqttServerConfigFooterViewModel alloc] init];
    }
