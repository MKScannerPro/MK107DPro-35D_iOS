//
//  MKSDMqttServerConfigFooterView.m
...
@implementation MKSDMqttServerConfigFooterViewModel
@end
...
@interface MKSDMqttServerConfigFooterView ()<UIScrollViewDelegate,
MKMQTTGeneralParamsViewDelegate,
MKMQTTUserCredentialsViewDelegate,
MKSDMqttServerSSLViewDelegate,
MKSDMqttServerSettingViewDelegate,
MKSDMqttServerLwtViewDelegate>
...
@property (nonatomic, strong)MKSDMqttServerSSLView *sslView;

@property (nonatomic, strong)MKSDMqttServerLwtView *lwtView;

@property (nonatomic, strong)MKSDMqttServerSettingView *settingView;

@property (nonatomic, assign)NSInteger index;

@end

@implementation MKSDMqttServerConfigFooterView
...
#pragma mark - MKSDMqttServerSSLViewDelegate
- (void)sd_mqtt_sslParams_modifyDevice_sslStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(sd_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
...
- (void)sd_mqtt_sslParams_modifyDevice_certificateChanged:(NSInteger)certificate {
...
/// @param value value
- (void)sd_mqtt_sslParams_modifyDevice_textFieldValueChanged:(NSInteger)index value:(NSString *)value {
    ...
    if ([self.delegate respondsToSelector:@selector(sd_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate sd_mqtt_serverForDevice_textFieldValueChanged:value textID:tempIndex];
    }
}

#pragma mark - MKSDMqttServerSettingViewDelegate
...
#pragma mark - MKSDMqttServerLwtViewDelegate
...
#pragma mark - setter
- (void)setDataModel:(MKSDMqttServerConfigFooterViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKSDMqttServerConfigFooterViewModel.class]) {
        return;
    }
    ...
    MKSDMqttServerSSLViewModel *sslModel = [[MKSDMqttServerSSLViewModel alloc] init];
    ...
    MKSDMqttServerLwtViewModel *lwtModel = [[MKSDMqttServerLwtViewModel alloc] init];
...
- (MKSDMqttServerSSLView *)sslView {
    if (!_sslView) {
        _sslView = [[MKSDMqttServerSSLView alloc] init];
...
- (MKSDMqttServerLwtView *)lwtView {
    if (!_lwtView) {
        _lwtView = [[MKSDMqttServerLwtView alloc] init];
...
- (MKSDMqttServerSettingView *)settingView {
    if (!_settingView) {
        _settingView = [[MKSDMqttServerSettingView alloc] init];
