...
@implementation MKSDMqttServerSSLViewModel
@end

@interface MKSDMqttServerSSLView ()<MKSDMqttServerSSLTextFieldDelegate>
...
@property (nonatomic, strong)MKSDMqttServerSSLTextField *caFileView;

@property (nonatomic, strong)MKSDMqttServerSSLTextField *clientKeyView;

@property (nonatomic, strong)MKSDMqttServerSSLTextField *clientCertView;

@end

@implementation MKSDMqttServerSSLView
...
#pragma mark - MKSDMqttServerSSLTextFieldDelegate
/// textField内容发送改变时的回调事件
/// @param index 当前cell所在的index
/// @param value 当前textField的值
- (void)sd_modifyServerSSLTextFieldValueChanged:(NSInteger)index textValue:(NSString *)value {
    if ([self.delegate respondsToSelector:@selector(sd_mqtt_sslParams_modifyDevice_textFieldValueChanged:value:)]) {
        [self.delegate sd_mqtt_sslParams_modifyDevice_textFieldValueChanged:index value:value];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(sd_mqtt_sslParams_modifyDevice_sslStatusChanged:)]) {
        [self.delegate sd_mqtt_sslParams_modifyDevice_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(sd_mqtt_sslParams_modifyDevice_certificateChanged:)]) {
            [self.delegate sd_mqtt_sslParams_modifyDevice_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKSDMqttServerSSLViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKSDMqttServerSSLViewModel.class]) {
        return;
    }
    ...
    MKSDMqttServerSSLTextFieldModel *caModel = [[MKSDMqttServerSSLTextFieldModel alloc] init];
    caModel.index = 0;
    ...
    MKSDMqttServerSSLTextFieldModel *clientKeyModel = [[MKSDMqttServerSSLTextFieldModel alloc] init];
    ...
    MKSDMqttServerSSLTextFieldModel *clientModel = [[MKSDMqttServerSSLTextFieldModel alloc] init];
    clientModel.index = 2;
...
- (void)updateSSLButtonIcon {
    UIImage *image = (self.sslButton.selected ? LOADICON(@"MKRemoteSevenDPro", @"MKSDMqttServerSSLView", @"sd_switchSelectedIcon.png") : LOADICON(@"MKRemoteSevenDPro", @"MKSDMqttServerSSLView", @"sd_switchUnselectedIcon.png"));
...
- (UIButton *)sslButton {
    if (!_sslButton) {
        ...
        [_sslButton setImage:LOADICON(@"MKRemoteSevenDPro", @"MKSDMqttServerSSLView", @"sd_switchUnselectedIcon.png") forState:UIControlStateNormal];
...
- (MKSDMqttServerSSLTextField *)caFileView {
    if (!_caFileView) {
        _caFileView = [[MKSDMqttServerSSLTextField alloc] init];
        _caFileView.delegate = self;
    }
    return _caFileView;
}

- (MKSDMqttServerSSLTextField *)clientKeyView {
    if (!_clientKeyView) {
        _clientKeyView = [[MKSDMqttServerSSLTextField alloc] init];
        _clientKeyView.delegate = self;
    }
    return _clientKeyView;
}

- (MKSDMqttServerSSLTextField *)clientCertView {
    if (!_clientCertView) {
        _clientCertView = [[MKSDMqttServerSSLTextField alloc] init];
