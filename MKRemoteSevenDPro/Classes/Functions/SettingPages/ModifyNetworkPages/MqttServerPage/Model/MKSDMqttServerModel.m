...
@implementation MKSDMqttServerModel
...
- (void)updateValue:(MKSDMqttServerModel *)model {
    if (!model || ![model isKindOfClass:MKSDMqttServerModel.class]) {
        return;
    }
...
- (NSInteger)readOTAState {
    ...
    [MKSDMQTTInterface sd_readOtaStatusWithMacAddress:[MKSDDeviceModeManager shared].macAddress topic:[MKSDDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (BOOL)readMqttInfos {
    ...
    [MKSDMQTTInterface sd_readMQTTParamsWithMacAddress:[MKSDDeviceModeManager shared].macAddress topic:[MKSDDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configMqttInfos {
    ...
    if ([self.lwtTopic isEqualToString:defaultPubTopic]) {
        //用户使用默认的LWT topic
        self.lwtTopic = [NSString stringWithFormat:@"%@/%@/%@",[MKSDDeviceModeManager shared].deviceName,self.clientID,@"device_to_app"];
    }
    if ([self.publishTopic isEqualToString:defaultPubTopic]) {
        //用户使用默认的topic
        self.publishTopic = [NSString stringWithFormat:@"%@/%@/%@",[MKSDDeviceModeManager shared].deviceName,self.clientID,@"device_to_app"];
    }
    if ([self.subscribeTopic isEqualToString:defaultSubTopic]) {
        //用户使用默认的topic
        self.subscribeTopic = [NSString stringWithFormat:@"%@/%@/%@",[MKSDDeviceModeManager shared].deviceName,self.clientID,@"app_to_device"];
    }
    if (!self.sslIsOn) {
        self.connectMode = 0;
    }else {
    ...
    [MKSDMQTTInterface sd_modifyMqttInfos:self macAddress:[MKSDDeviceModeManager shared].macAddress topic:[MKSDDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)configMqttCerts {
    __block BOOL success = NO;
    [MKSDMQTTInterface sd_modifyMqttCerts:self macAddress:[MKSDDeviceModeManager shared].macAddress topic:[MKSDDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
