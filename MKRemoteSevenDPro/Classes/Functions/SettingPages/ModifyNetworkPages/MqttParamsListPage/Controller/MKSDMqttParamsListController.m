//
//  MKSDMqttParamsListController.m
...
@interface MKSDMqttParamsListController ()<UITableViewDelegate, UITableViewDataSource>
...
@property (nonatomic, strong)MKSDMqttParamsModel *dataModel;

@end

@implementation MKSDMqttParamsListController

- (void)dealloc {
    NSLog(@"MKSDMqttParamsListController销毁");
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0) {
        //WIFI Settings
        MKSDMqttWifiSettingsController *vc = [[MKSDMqttWifiSettingsController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 1) {
        //MQTT Settings
        MKSDMqttServerController *vc = [[MKSDMqttServerController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 2) {
        //Network Settings
        MKSDMqttNetworkSettingsController *vc = [[MKSDMqttNetworkSettingsController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
}
...
- (void)updateLocaDeviceData {
    [[MKHudManager share] showHUDWithTitle:@"Waiting..." inView:self.view isPenetration:NO];
    [MKSDDeviceDatabaseManager updateClientID:self.dataModel.clientID subscribedTopic:self.dataModel.subscribeTopic publishedTopic:self.dataModel.publishTopic lwtStatus:self.dataModel.lwtStatus lwtTopic:self.dataModel.lwtTopic macAddress:[MKSDDeviceModeManager shared].macAddress sucBlock:^{
        [[MKHudManager share] hide];
        [self.view showCentralToast:@"Setup succeed!"];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_sd_deviceModifyMQTTServerSuccessNotification"
                                                            object:nil
                                                          userInfo:@{
            @"macAddress":SafeStr([MKSDDeviceModeManager shared].macAddress),
...
- (MKSDMqttParamsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKSDMqttParamsModel alloc] init];
    }
    return _dataModel;
}
