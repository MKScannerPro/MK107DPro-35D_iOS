//
//  MKSDMqttWifiSettingsController.m
...
@interface MKSDMqttWifiSettingsController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextButtonCellDelegate,
MKTextFieldCellDelegate,
mk_textSwitchCellDelegate,
MKSDWifiSettingsBandCellDelegate>
...
@property (nonatomic, strong)MKSDMqttWifiSettingsModel *dataModel;

@end

@implementation MKSDMqttWifiSettingsController

- (void)dealloc {
    NSLog(@"MKSDMqttWifiSettingsController销毁");
}
...
#pragma mark - MKSDWifiSettingsBandCellDelegate
- (void)sd_wifiSettingsBandCell_countryChanged:(NSInteger)country {
    MKSDWifiSettingsBandCellModel *cellModel = self.section11List[0];
            ...
            if (((self.dataModel.eapType == 0 || self.dataModel.eapType == 1) && self.dataModel.verifyServer) || self.dataModel.eapType == 2) {
                ...
                [[NSNotificationCenter defaultCenter] addObserver:self
                                                         selector:@selector(receiveUpdateEAPCerts:)
                                                             name:MKSDReceiveDeviceUpdateEapCertsResultNotification
                                                           object:nil];
                return;
            }
        }
...
- (UITableViewCell *)cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    //Country&Band
    MKSDWifiSettingsBandCell *cell = [MKSDWifiSettingsBandCell initCellWithTableView:self.tableView];
...
- (void)loadSection11Datas {
    MKSDWifiSettingsBandCellModel *cellModel = [[MKSDWifiSettingsBandCellModel alloc] init];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKRemoteSevenDPro", @"MKSDMqttWifiSettingsController", @"sd_saveIcon.png") forState:UIControlStateNormal];
...
- (MKSDMqttWifiSettingsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKSDMqttWifiSettingsModel alloc] init];
    }
    return _dataModel;
}
