...
@implementation MKSDWifiSettingsBandCellModel
@end

@interface MKSDWifiSettingsBandCell ()
...
@implementation MKSDWifiSettingsBandCell

+ (MKSDWifiSettingsBandCell *)initCellWithTableView:(UITableView *)tableView {
    MKSDWifiSettingsBandCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKSDWifiSettingsBandCellIdenty"];
    if (!cell) {
        cell = [[MKSDWifiSettingsBandCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKSDWifiSettingsBandCellIdenty"];
    }
    return cell;
}
...
- (void)selectedButtonPressed {
    ...
    [pickView showPickViewWithDataList:self.dataList selectedRow:row block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(sd_wifiSettingsBandCell_countryChanged:)]) {
            [self.delegate sd_wifiSettingsBandCell_countryChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKSDWifiSettingsBandCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKSDWifiSettingsBandCellModel.class] || _dataModel.country >= self.dataList.count) {
        return;
    }
