...
@interface MKSDSystemTimeController ()<UITableViewDelegate,
UITableViewDataSource,
MKSDSystemTimeCellDelegate,
...
@implementation MKSDSystemTimeController

- (void)dealloc {
    NSLog(@"MKSDSystemTimeController销毁");
    if (self.readTimer) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0)  {
        //Sync Time From NTP
        MKSDNTPServerController *vc = [[MKSDNTPServerController alloc] init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKSDSystemTimeCell *cell = [MKSDSystemTimeCell initCellWithTableView:tableView];
...
#pragma mark - MKSDSystemTimeCellDelegate
...
- (void)readDataFromServer {
    ...
    [MKSDMQTTInterface sd_readDeviceUTCTimeWithMacAddress:[MKSDDeviceModeManager shared].macAddress topic:[MKSDDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)syncTimeZoneToDevice:(NSInteger)timeZone {
    ...
    [MKSDMQTTInterface sd_configDeviceTimeZone:timeZone - 24 timestamp:[[NSDate date] timeIntervalSince1970] macAddress:[MKSDDeviceModeManager shared].macAddress topic:[MKSDDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)loadSection1Datas {
    MKSDSystemTimeCellModel *cellModel = [[MKSDSystemTimeCellModel alloc] init];
    cellModel.index = 0;
