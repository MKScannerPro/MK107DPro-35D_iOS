...
@interface MKSDAdvBeaconController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKSDAdvBeaconModel *dataModel;

@end

@implementation MKSDAdvBeaconController

- (void)dealloc {
    NSLog(@"MKSDAdvBeaconController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKRemoteSevenDPro", @"MKSDAdvBeaconController", @"sd_saveIcon.png") forState:UIControlStateNormal];
...
- (MKSDAdvBeaconModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKSDAdvBeaconModel alloc] init];
    }
    return _dataModel;
}
