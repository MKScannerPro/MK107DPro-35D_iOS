...
@interface MKSDIndicatorSetingsModel ()
...
@implementation MKSDIndicatorSetingsModel
...
- (BOOL)readIndicatorLightStatus {
    ...
    [MKSDMQTTInterface sd_readIndicatorLightStatusWithMacAddress:[MKSDDeviceModeManager shared].macAddress topic:[MKSDDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        ...
        self.power_led = [returnData[@"data"][@"power_led"] boolValue];
...
- (BOOL)configIndicatorLightStatus {
    __block BOOL success = NO;
    [MKSDMQTTInterface sd_configIndicatorLightStatus:self macAddress:[MKSDDeviceModeManager shared].macAddress topic:[MKSDDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
