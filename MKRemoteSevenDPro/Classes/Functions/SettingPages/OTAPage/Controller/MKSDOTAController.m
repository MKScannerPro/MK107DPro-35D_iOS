...
@interface MKSDOTAController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKSDOTAPageModel *dataModel;

@end

@implementation MKSDOTAController

- (void)dealloc {
    NSLog(@"MKSDOTAController销毁");
        ...
        if (self.dataModel.otaType == 0) {
            [[NSNotificationCenter defaultCenter] addObserver:self
                                                     selector:@selector(receiveOTAResult:)
                                                         name:MKSDReceiveDeviceOTAResultNotification
                                                       object:nil];
        }else if (self.dataModel.otaType == 1) {
            [[NSNotificationCenter defaultCenter] addObserver:self
                                                     selector:@selector(receiveOTAResult:)
                                                         name:MKSDReceiveDeviceNpcOTAResultNotification
                                                       object:nil];
        }
...
- (MKSDOTAPageModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKSDOTAPageModel alloc] init];
    }
    return _dataModel;
}
