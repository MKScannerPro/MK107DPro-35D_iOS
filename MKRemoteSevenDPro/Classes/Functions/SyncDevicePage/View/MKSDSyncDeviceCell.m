...
@implementation MKSDSyncDeviceCellModel
@end

@interface MKSDSyncDeviceCell ()
...
@implementation MKSDSyncDeviceCell

+ (MKSDSyncDeviceCell *)initCellWithTableView:(UITableView *)tableView {
    MKSDSyncDeviceCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKSDSyncDeviceCellIdenty"];
    if (!cell) {
        cell = [[MKSDSyncDeviceCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKSDSyncDeviceCellIdenty"];
    }
    return cell;
}
...
- (void)backBtnPressed {
    ...
    [self updateIcon];
    if ([self.delegate respondsToSelector:@selector(sd_syncDeviceCell_selected:index:)]) {
        [self.delegate sd_syncDeviceCell_selected:self.backBtn.selected index:self.dataModel.index];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKSDSyncDeviceCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKSDSyncDeviceCellModel.class]) {
        return;
    }
...
- (void)updateIcon {
    UIImage *btnIcon = (self.backBtn.selected ? LOADICON(@"MKRemoteSevenDPro", @"MKSDSyncDeviceCell", @"sd_listButtonSelectedIcon.png") : LOADICON(@"MKRemoteSevenDPro", @"MKSDSyncDeviceCell", @"sd_listButtonUnselectedIcon.png"));
