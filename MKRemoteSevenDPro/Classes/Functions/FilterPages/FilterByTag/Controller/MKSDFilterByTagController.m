...
@interface MKSDFilterByTagController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKSDFilterByTagModel *dataModel;

@end

@implementation MKSDFilterByTagController

- (void)dealloc {
    NSLog(@"MKSDFilterByTagController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKRemoteSevenDPro", @"MKSDFilterByTagController", @"sd_saveIcon.png") forState:UIControlStateNormal];
...
- (MKSDFilterByTagModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKSDFilterByTagModel alloc] init];
    }
    return _dataModel;
}
