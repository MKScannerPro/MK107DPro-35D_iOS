//
//  MKSDFilterByAdvNameController.m
...
@interface MKSDFilterByAdvNameController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKSDFilterByAdvNameModel *dataModel;

@end

@implementation MKSDFilterByAdvNameController

- (void)dealloc {
    NSLog(@"MKSDFilterByAdvNameController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKRemoteSevenDPro", @"MKSDFilterByAdvNameController", @"sd_saveIcon.png") forState:UIControlStateNormal];
...
- (MKSDFilterByAdvNameModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKSDFilterByAdvNameModel alloc] init];
    }
    return _dataModel;
}
