...
@interface MKSDUploadOptionController ()<UITableViewDelegate,
UITableViewDataSource,
MKNormalSliderCellDelegate,
MKSDFilterCellDelegate>
...
@property (nonatomic, strong)MKSDUploadOptionModel *dataModel;

@end

@implementation MKSDUploadOptionController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKSDFilterCell *cell = [MKSDFilterCell initCellWithTableView:tableView];
...
- (void)sd_filterValueChanged:(NSInteger)dataListIndex index:(NSInteger)index {
    ...
    if (index == 1) {
        ...
        MKSDFilterCellModel *cellModel = self.section3List[0];
        cellModel.dataListIndex = dataListIndex;
        return;
    }
}
...
- (void)filterByMACAddress {
    MKSDFilterByMacController *vc = [[MKSDFilterByMacController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}

- (void)filterByADVName {
    MKSDFilterByAdvNameController *vc = [[MKSDFilterByAdvNameController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}

- (void)filterByRawData {
    MKSDFilterByRawDataController *vc = [[MKSDFilterByRawDataController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}

- (void)duplicateDataFilter {
    MKSDDuplicateDataFilterController *vc = [[MKSDDuplicateDataFilterController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}

- (void)uploadDataOption {
    MKSDUploadDataOptionController *vc = [[MKSDUploadDataOptionController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}
...
- (void)loadSection1Datas {
    MKSDFilterCellModel *cellModel = [[MKSDFilterCellModel alloc] init];
    cellModel.index = 0;
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKRemoteSevenDPro", @"MKSDUploadOptionController", @"sd_saveIcon.png")
...
- (MKSDUploadOptionModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKSDUploadOptionModel alloc] init];
    }
    return _dataModel;
}
