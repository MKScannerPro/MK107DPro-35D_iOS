...
@implementation MKSDFilterCellModel
@end

@interface MKSDFilterCell ()
...
@implementation MKSDFilterCell

+ (MKSDFilterCell *)initCellWithTableView:(UITableView *)tableView {
    MKSDFilterCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKSDFilterCellIdenty"];
    if (!cell) {
        cell = [[MKSDFilterCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKSDFilterCellIdenty"];
    }
    return cell;
}
...
- (void)rightButtonPressed {
    ...
    [pickView showPickViewWithDataList:self.dataModel.dataList selectedRow:row block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(sd_filterValueChanged:index:)]) {
            [self.delegate sd_filterValueChanged:currentRow index:self.dataModel.index];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKSDFilterCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKSDFilterCellModel.class]) {
        return;
    }
