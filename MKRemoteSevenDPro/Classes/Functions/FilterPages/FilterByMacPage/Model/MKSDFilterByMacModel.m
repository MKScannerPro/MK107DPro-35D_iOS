...
@interface MKSDFilterByMacModel ()
...
@implementation MKSDFilterByMacModel
...
- (BOOL)readFilterByMac {
    ...
    [MKSDMQTTInterface sd_readFilterByMacWithMacAddress:[MKSDDeviceModeManager shared].macAddress topic:[MKSDDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterByMac:(NSArray <NSString *>*)macList {
    __block BOOL success = NO;
    [MKSDMQTTInterface sd_configFilterByMacAddress:macList preciseMatch:self.preciseMatch reverseFilter:self.reverseFilter macAddress:[MKSDDeviceModeManager shared].macAddress topic:[MKSDDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
