//
//  MKSDDuplicateDataFilterModel.m
...
@interface MKSDDuplicateDataFilterModel ()
...
@implementation MKSDDuplicateDataFilterModel
...
- (BOOL)readDuplicateDataFilter {
    ...
    [MKSDMQTTInterface sd_readDuplicateDataFilterDatasWithMacAddress:[MKSDDeviceModeManager shared].macAddress topic:[MKSDDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configDuplicateDataFilter {
    __block BOOL success = NO;
    [MKSDMQTTInterface sd_configDuplicateDataFilter:self.rule period:[self.time longLongValue] macAddress:[MKSDDeviceModeManager shared].macAddress topic:[MKSDDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
