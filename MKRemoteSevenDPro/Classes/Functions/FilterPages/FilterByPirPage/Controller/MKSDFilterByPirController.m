...
@interface MKSDFilterByPirController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKSDFilterByPirModel *dataModel;

@end

@implementation MKSDFilterByPirController

- (void)dealloc {
    NSLog(@"MKSDFilterByPirController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKRemoteSevenDPro", @"MKSDFilterByPirController", @"sd_saveIcon.png") forState:UIControlStateNormal];
...
- (MKSDFilterByPirModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKSDFilterByPirModel alloc] init];
    }
    return _dataModel;
}
