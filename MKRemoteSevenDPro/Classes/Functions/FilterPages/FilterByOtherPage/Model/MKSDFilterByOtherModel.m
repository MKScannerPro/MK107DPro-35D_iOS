...
@implementation MKSDFilterRawAdvDataModel
...
@interface MKSDFilterByOtherModel ()
...
@implementation MKSDFilterByOtherModel
...
- (void)configWithRawDataList:(NSArray <MKSDFilterRawAdvDataModel *>*)list
...
- (BOOL)readFilterByOther {
    ...
    [MKSDMQTTInterface sd_readFilterOtherDatasWithMacAddress:[MKSDDeviceModeManager shared].macAddress topic:[MKSDDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterByOther:(NSArray <MKSDFilterRawAdvDataModel *>*)list relationship:(mk_sd_filterByOther)relationship {
    __block BOOL success = NO;
    [MKSDMQTTInterface sd_configFilterByOtherDatas:self.isOn relationship:relationship rawDataList:list macAddress:[MKSDDeviceModeManager shared].macAddress topic:[MKSDDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
