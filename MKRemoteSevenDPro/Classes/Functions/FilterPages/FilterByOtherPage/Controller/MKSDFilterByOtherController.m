...
@interface MKSDFilterByOtherController ()<UITableViewDelegate,
UITableViewDataSource,
mk_textSwitchCellDelegate,
MKSDFilterEditSectionHeaderViewDelegate,
MKSDFilterByRawDataCellDelegate,
...
@property (nonatomic, strong)MKSDFilterByOtherModel *dataModel;

@end

@implementation MKSDFilterByOtherController

- (void)dealloc {
    NSLog(@"MKSDFilterByOtherController销毁");
}
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    if (section == 1) {
        ...
        MKSDFilterEditSectionHeaderViewModel *model = [[MKSDFilterEditSectionHeaderViewModel alloc] init];
        model.index = 0;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKSDFilterByRawDataCell *cell = [MKSDFilterByRawDataCell initCellWithTableView:tableView];
...
- (void)mk_sd_filterEditSectionHeaderView_addButtonPressed:(NSInteger)index {
    ...
    MKSDFilterByRawDataCellModel *cellModel = [[MKSDFilterByRawDataCellModel alloc] init];
    if (cellModelIndex == 0) {
        //Condition A
...
#pragma mark - MKSDFilterByRawDataCellDelegate
/// 输入框内容发生改变
...
- (void)sd_rawFilterDataChanged:(mk_sd_filterRawAdvDataTextType)textType
    ...
    MKSDFilterByRawDataCellModel *cellModel = self.section1List[index];
    if (textType == mk_sd_filterRawAdvDataTextTypeDataType) {
        //过滤类型输入框内容发生改变
        cellModel.dataType = textValue;
        return;
    }
    if (textType == mk_sd_filterRawAdvDataTextTypeMinIndex) {
        //开始过滤的Byte索引输入框发生改变
        cellModel.minIndex = textValue;
        return;
    }
    if (textType == mk_sd_filterRawAdvDataTextTypeMaxIndex) {
        //截止过滤的Byte索引输入框发生改变
        cellModel.maxIndex = textValue;
        return;
    }
    if (textType == mk_sd_filterRawAdvDataTextTypeRawDataType) {
        //过滤内容输入框发生改变
...
- (void)configDataToDevice {
    ...
    for (NSInteger i = 0; i < self.section1List.count; i ++) {
        MKSDFilterByRawDataCellModel *cellModel = self.section1List[i];
        ...
        MKSDFilterRawAdvDataModel *model = [[MKSDFilterRawAdvDataModel alloc] init];
...
- (mk_sd_filterByOther)loadCurrentRelationship {
    MKTextButtonCellModel *cellModel = self.section2List[0];
    if (self.section1List.count == 1) {
        //@[@"A"];
        return mk_sd_filterByOther_A;
    }
    if (self.section1List.count == 2) {
        //@[@"A & B",@"A | B"]
        if (cellModel.dataListIndex == 0) {
            return mk_sd_filterByOther_AB;
        }
        return mk_sd_filterByOther_AOrB;
    }
    if (self.section1List.count == 3) {
        //@[@"A & B & C",@"(A & B) | C",@"A | B | C"]
        if (cellModel.dataListIndex == 0) {
            return mk_sd_filterByOther_ABC;
        }
        if (cellModel.dataListIndex == 1) {
            return mk_sd_filterByOther_ABOrC;
        }
        if (cellModel.dataListIndex == 2) {
            return mk_sd_filterByOther_AOrBOrC;
        }
    }
    return mk_sd_filterByOther_AOrBOrC;
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKRemoteSevenDPro", @"MKSDFilterByOtherController", @"sd_saveIcon.png") forState:UIControlStateNormal];
...
- (MKSDFilterByOtherModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKSDFilterByOtherModel alloc] init];
    }
    return _dataModel;
}
