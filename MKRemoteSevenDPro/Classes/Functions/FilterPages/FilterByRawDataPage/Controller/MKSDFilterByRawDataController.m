//
//  MKSDFilterByRawDataController.m
...
@interface MKSDFilterByRawDataController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKSDFilterByRawDataModel *dataModel;

@end

@implementation MKSDFilterByRawDataController

- (void)dealloc {
    NSLog(@"MKSDFilterByRawDataController销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0) {
        //iBeacon
        MKSDFilterByBeaconController *vc = [[MKSDFilterByBeaconController alloc] init];
        vc.pageType = mk_sd_filterByBeaconPageType_beacon;
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 1) {
        //Eddystone-UID
        MKSDFilterByUIDController *vc = [[MKSDFilterByUIDController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 2) {
        //Eddystone-URL
        MKSDFilterByURLController *vc = [[MKSDFilterByURLController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 3) {
        //Eddystone-TLM
        MKSDFilterByTLMController *vc = [[MKSDFilterByTLMController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 2 && indexPath.row == 0) {
        //BXP-Button
        MKSDFilterByButtonController *vc = [[MKSDFilterByButtonController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 2 && indexPath.row == 1) {
        //BXP-Tag
        MKSDFilterByTagController *vc = [[MKSDFilterByTagController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 2 && indexPath.row == 2) {
        //PIR
        MKSDFilterByPirController *vc = [[MKSDFilterByPirController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 2 && indexPath.row == 3) {
        //Other
        MKSDFilterByOtherController *vc = [[MKSDFilterByOtherController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
}
...
- (void)configFilterBXPDeviceInfo:(BOOL)isOn {
    ...
    [MKSDMQTTInterface sd_configFilterFilterBXPDeviceInfo:isOn macAddress:[MKSDDeviceModeManager shared].macAddress topic:[MKSDDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
...
- (void)configFilterBXPACC:(BOOL)isOn {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKSDMQTTInterface sd_configFilterBXPAcc:isOn macAddress:[MKSDDeviceModeManager shared].macAddress topic:[MKSDDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
...
- (void)configFilterBXPTH:(BOOL)isOn {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKSDMQTTInterface sd_configFilterBXPTH:isOn macAddress:[MKSDDeviceModeManager shared].macAddress topic:[MKSDDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
...
- (MKSDFilterByRawDataModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKSDFilterByRawDataModel alloc] init];
    }
    return _dataModel;
}
