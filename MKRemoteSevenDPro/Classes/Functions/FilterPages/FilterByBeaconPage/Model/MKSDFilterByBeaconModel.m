...
@interface MKSDFilterByBeaconModel ()

@property (nonatomic, strong)dispatch_queue_t readQueue;

@property (nonatomic, strong)dispatch_semaphore_t semaphore;

@property (nonatomic, assign)mk_sd_filterByBeaconPageType pageType;

@end

@implementation MKSDFilterByBeaconModel
...
- (void)readDataWithSucBlock:(void (^)(void))sucBlock failedBlock:(void (^)(NSError *error))failedBlock {
    dispatch_async(self.readQueue, ^{
        if (self.pageType == mk_sd_filterByBeaconPageType_beacon) {
        ...
        /*
        else if (self.pageType == mk_sd_filterByBeaconPageType_MKBeacon) {
            if (![self readFilterByMKBeacon]) {
                [self operationFailedBlockWithMsg:@"Read Filter iBeacon Error" block:failedBlock];
                return;
            }
        }else if (self.pageType == mk_sd_filterByBeaconPageType_MKBeaconAcc) {
...
- (BOOL)readFilterByBeacon {
    ...
    [MKSDMQTTInterface sd_readFilterByBeaconWithMacAddress:[MKSDDeviceModeManager shared].macAddress topic:[MKSDDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterByBeacon {
    ...
    [MKSDMQTTInterface sd_configFilterByBeacon:self.isOn minMinor:tempMinMinor maxMinor:tempMaxMinor minMajor:tempMinMajor maxMajor:tempMaxMajor uuid:self.uuid macAddress:[MKSDDeviceModeManager shared].macAddress topic:[MKSDDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readFilterByMKBeacon {
    __block BOOL success = NO;
    [MKSDMQTTInterface sd_readFilterByMKBeaconWithDeviceID:self.deviceID macAddress:self.macAddress topic:self.topic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)readFilterByMKBeaconACC {
    __block BOOL success = NO;
    [MKSDMQTTInterface sd_readFilterByMKBeaconACCWithDeviceID:self.deviceID macAddress:self.macAddress topic:self.topic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        self.isOn = ([returnData[@"data"][@"switch"] integerValue] == 1);
        self.minMinor = [NSString stringWithFormat:@"%@",returnData[@"data"][@"min_minor"]];
        self.maxMinor = [NSString stringWithFormat:@"%@",returnData[@"data"][@"max_minor"]];
...
- (BOOL)configFilterByMKBeaconACC {
    __block BOOL success = NO;
    [MKSDMQTTInterface sd_configFilterByMKBeaconACC:self.isOn minMinor:[self.minMinor integerValue] maxMinor:[self.maxMinor integerValue] minMajor:[self.minMajor integerValue] maxMajor:[self.maxMajor integerValue] uuid:self.uuid deviceID:self.deviceID macAddress:self.macAddress topic:self.topic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
