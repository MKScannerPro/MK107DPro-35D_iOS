...
@implementation MKSDDeviceListCell

+ (MKSDDeviceListCell *)initCellWithTableView:(UITableView *)tableView {
    MKSDDeviceListCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKSDDeviceListCellIdenty"];
    if (!cell) {
        cell = [[MKSDDeviceListCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKSDDeviceListCellIdenty"];
    }
    return cell;
}
...
- (void)longPressEventAction:(UILongPressGestureRecognizer *)longPress {
    if (longPress.state == UIGestureRecognizerStateBegan && [self.delegate respondsToSelector:@selector(sd_cellDeleteButtonPressed:)]) {
        [self.delegate sd_cellDeleteButtonPressed:self.indexPath.row];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKSDDeviceListModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKSDDeviceModel.class]) {
        return;
    }
    self.deviceNameLabel.text = SafeStr(_dataModel.deviceName);
    self.stateLabel.text = (_dataModel.onLineState == MKSDDeviceModelStateOnline ? @"Online" : @"Offline");
    self.stateLabel.textColor = (_dataModel.onLineState == MKSDDeviceModelStateOnline ? NAVBAR_COLOR_MACROS : UIColorFromRGB(0xcccccc));
    self.macLabel.text = SafeStr(_dataModel.macAddress);
    if (dataModel.onLineState == MKSDDeviceModelStateOffline) {
        //设备离线
        self.wifiIcon.image = LOADICON(@"MKRemoteSevenDPro", @"MKSDDeviceListCell", @"sd_offline_wifi.png");
        return;
    }
    //设备在线
    if (_dataModel.wifiLevel >= -50) {
        //Good
        self.wifiIcon.image = LOADICON(@"MKRemoteSevenDPro", @"MKSDDeviceListCell", @"sd_good_wifi.png");
        return;
    }
    if (_dataModel.wifiLevel >= -65 && _dataModel.wifiLevel < -50) {
        //Medium
        self.wifiIcon.image = LOADICON(@"MKRemoteSevenDPro", @"MKSDDeviceListCell", @"sd_medium_wifi.png");
        return;
    }
    if (_dataModel.wifiLevel < -65) {
        //Poor
        self.wifiIcon.image = LOADICON(@"MKRemoteSevenDPro", @"MKSDDeviceListCell", @"sd_poor_wifi.png");
        return;
    }
}
...
- (UIImageView *)wifiIcon {
    if (!_wifiIcon) {
        ...
        _wifiIcon.image = LOADICON(@"MKRemoteSevenDPro", @"MKSDDeviceListCell", @"sd_offline_wifi.png");
    }
    return _wifiIcon;
}
...
- (UIImageView *)rightIcon {
    if (!_rightIcon) {
        ...
        _rightIcon.image = LOADICON(@"MKRemoteSevenDPro", @"MKSDDeviceListCell", @"sd_goNextButton.png");
    }
    return _rightIcon;
}
