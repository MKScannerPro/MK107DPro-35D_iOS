...
static NSString *const localPasswordKey = @"mk_sd_passwordKey";

static NSTimeInterval const kRefreshInterval = 0.5f;

@interface MKSDScanPageController ()<UITableViewDelegate,
UITableViewDataSource,
mk_sd_centralManagerScanDelegate>
...
@implementation MKSDScanPageController

- (void)dealloc {
    NSLog(@"MKSDScanPageController销毁");
    //移除runloop的监听
    CFRunLoopRemoveObserver(CFRunLoopGetCurrent(), self.observerRef, kCFRunLoopCommonModes);
    [[MKSDCentralManager shared] stopScan];
    [MKSDCentralManager sharedDealloc];
}
...
- (void)viewDidLoad {
    ...
    [MKSDCentralManager shared].delegate = self;
}

#pragma mark - super method
- (void)rightButtonMethod {
    if ([MKSDCentralManager shared].centralStatus != mk_sd_centralManagerStatusEnable) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    MKSDScanPageCell *cell = [MKSDScanPageCell initCellWithTableView:tableView];
    cell.dataModel = self.dataList[indexPath.row];
    return cell;
}

#pragma mark - mk_sd_centralManagerScanDelegate

- (void)mk_sd_receiveDevice:(NSDictionary *)deviceModel {
    MKSDScanPageModel *dataModel = [MKSDScanPageModel mk_modelWithJSON:deviceModel];
...
- (void)scanTimerRun{
    ...
    [[MKSDCentralManager shared] startScan];
...
- (void)updateDataWithDeviceModel:(MKSDScanPageModel *)deviceModel{
...
#pragma mark - 连接部分
- (void)connectDeviceWithModel:(MKSDScanPageModel *)deviceModel {
    //停止扫描
    [self.refreshIcon.layer removeAnimationForKey:@"mk_refreshAnimationKey"];
    [[MKSDCentralManager shared] stopScan];
    if (self.scanTimer) {
...
- (void)connectDevice:(MKSDScanPageModel *)deviecModel {
    ...
    [[MKSDCentralManager shared] connectPeripheral:deviecModel.peripheral password:password sucBlock:^(CBPeripheral * _Nonnull peripheral) {
...
- (void)pushMQTTForDevicePage:(NSString *)deviceType {
    MKSDDeviceParamsListController *vc = [[MKSDDeviceParamsListController alloc] init];
...
- (UIImageView *)refreshIcon {
    if (!_refreshIcon) {
        ...
        _refreshIcon.image = LOADICON(@"MKRemoteSevenDPro", @"MKSDScanPageController", @"sd_scanRefresh.png");
    }
    return _refreshIcon;
}
