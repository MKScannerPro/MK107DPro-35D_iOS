...
@implementation MKSDScanPageCell

+ (MKSDScanPageCell *)initCellWithTableView:(UITableView *)tableView {
    MKSDScanPageCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKSDScanPageCellIdenty"];
    if (!cell) {
        cell = [[MKSDScanPageCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKSDScanPageCellIdenty"];
    }
    return cell;
}
...
#pragma mark - setter
- (void)setDataModel:(MKSDScanPageModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKSDScanPageModel.class]) {
        return;
    }
