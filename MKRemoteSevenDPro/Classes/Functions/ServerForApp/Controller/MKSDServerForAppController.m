...
@interface MKSDServerForAppController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
MKSDServerConfigAppFooterViewDelegate,
...
@property (nonatomic, strong)MKSDServerForAppModel *dataModel;

@property (nonatomic, strong)MKSDServerConfigAppFooterView *sslParamsView;

@property (nonatomic, strong)MKSDServerConfigAppFooterViewModel *sslParamsModel;
...
@implementation MKSDServerForAppController

- (void)dealloc {
    NSLog(@"MKSDServerForAppController销毁");
...
- (void)rightButtonMethod {
    ...
    [[MKSDMQTTDataManager shared] saveServerParams:self.dataModel];
    ...
    [[MKSDMQTTDataManager shared] connect];
}
...
#pragma mark - MKSDServerConfigAppFooterViewDelegate
/// 用户改变了开关状态
/// @param isOn isOn
/// @param statusID 0:cleanSession   1:ssl
- (void)sd_mqtt_serverForApp_switchStatusChanged:(BOOL)isOn statusID:(NSInteger)statusID {
    if (statusID == 0) {
        //cleanSession
...
- (void)sd_mqtt_serverForApp_qosChanged:(NSInteger)qos {
...
- (void)sd_mqtt_serverForApp_textFieldValueChanged:(NSString *)text textID:(NSInteger)textID {
    if (textID == 0) {
        //keep alive
...
- (void)sd_mqtt_serverForApp_certificateChanged:(NSInteger)certificate {
...
- (void)sd_mqtt_serverForApp_fileButtonPressed:(NSInteger)fileType {
    if (fileType == 0) {
        //caFaile
...
- (void)sd_mqtt_serverForApp_bottomButtonPressed:(NSInteger)index {
    if (index == 0) {
        //Export Demo File
...
- (void)sd_selectedServerParams:(NSString *)fileName {
    [MKSDExcelDataManager parseAppExcel:fileName
                                sucBlock:^(NSDictionary * _Nonnull returnData) {
        MKSDServerForAppModel *model = [MKSDServerForAppModel mk_modelWithJSON:returnData];
...
- (void)serverManagerStateChanged {
    ...
    if ([MKSDMQTTDataManager shared].state == MKSDMQTTSessionManagerStateConnected) {
        ...
        [[NSNotificationCenter defaultCenter] removeObserver:self
                                                        name:MKSDMQTTSessionManagerStateChangedNotification
                                                      object:nil];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_sd_needReloadTopicsNotification"
                                                            object:nil];
...
//    }
    ...
    [MKSDExcelDataManager exportAppExcel:self.dataModel
                                 sucBlock:^{
        [[MKHudManager share] hide];
        [self sharedExcel];
    }
...
- (void)connectFailed {
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self
                                                    name:MKSDMQTTSessionManagerStateChangedNotification
                                                  object:nil];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"MKSDMQTTServerConnectFailedNotification" object:nil];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKRemoteSevenDPro", @"MKSDServerForAppController", @"sd_saveIcon.png") forState:UIControlStateNormal];
...
- (MKSDServerForAppModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKSDServerForAppModel alloc] init];
    }
    return _dataModel;
}
...
- (MKSDServerConfigAppFooterView *)sslParamsView {
    if (!_sslParamsView) {
        _sslParamsView = [[MKSDServerConfigAppFooterView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 230.f)];
...
- (MKSDServerConfigAppFooterViewModel *)sslParamsModel {
    if (!_sslParamsModel) {
        _sslParamsModel = [[MKSDServerConfigAppFooterViewModel alloc] init];
    }
